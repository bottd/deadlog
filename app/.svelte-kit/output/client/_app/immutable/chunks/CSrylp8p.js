var e = Object.defineProperty,
	t = (t, n) => {
		let r = {};
		for (var i in t) e(r, i, { get: t[i], enumerable: !0 });
		return (n || e(r, Symbol.toStringTag, { value: `Module` }), r);
	},
	n = Array.isArray,
	r = Array.prototype.indexOf,
	i = Array.prototype.includes,
	a = Array.from,
	o = Object.defineProperty,
	s = Object.getOwnPropertyDescriptor,
	c = Object.getOwnPropertyDescriptors,
	l = Object.prototype,
	u = Array.prototype,
	d = Object.getPrototypeOf,
	f = Object.isExtensible;
function p(e) {
	return typeof e == `function`;
}
var m = () => {};
function h(e) {
	return e();
}
function g(e) {
	for (var t = 0; t < e.length; t++) e[t]();
}
function _() {
	var e, t;
	return {
		promise: new Promise((n, r) => {
			((e = n), (t = r));
		}),
		resolve: e,
		reject: t
	};
}
function v(e, t, n = !1) {
	return e === void 0 ? (n ? t() : t) : e;
}
function y(e, t) {
	if (Array.isArray(e)) return e;
	if (t === void 0 || !(Symbol.iterator in e)) return Array.from(e);
	let n = [];
	for (let r of e) if ((n.push(r), n.length === t)) break;
	return n;
}
var b = 1 << 24,
	x = 1024,
	S = 2048,
	C = 4096,
	ee = 8192,
	te = 16384,
	ne = 32768,
	re = 1 << 25,
	ie = 65536,
	ae = 1 << 18,
	oe = 1 << 19,
	se = 1 << 20,
	ce = 1 << 25,
	le = 65536,
	ue = 1 << 21,
	de = 1 << 22,
	fe = 1 << 23,
	pe = Symbol(`$state`),
	me = Symbol(`component`),
	he = Symbol(`legacy props`),
	ge = Symbol(``),
	_e = Symbol(`proxy path`),
	ve = Symbol(`attributes`),
	ye = Symbol(`class`),
	be = Symbol(`style`),
	xe = Symbol(`text`),
	Se = Symbol(`form reset`),
	Ce = new (class extends Error {
		name = `StaleReactionError`;
		message = 'The reaction that called `getAbortSignal()` was re-run or destroyed';
	})(),
	we =
		!!globalThis.document?.contentType && globalThis.document.contentType.includes(`xml`),
	Te = {},
	w = Symbol(`uninitialized`),
	Ee = `http://www.w3.org/1999/xhtml`,
	De = `http://www.w3.org/2000/svg`,
	Oe = `http://www.w3.org/1998/Math/MathML`,
	ke = `@attach`;
function Ae() {
	console.warn(`https://svelte.dev/e/derived_inert`);
}
function je(e) {
	console.warn(`https://svelte.dev/e/hydratable_missing_but_expected`);
}
function Me(e) {
	console.warn(`https://svelte.dev/e/hydration_mismatch`);
}
function Ne() {
	console.warn(`https://svelte.dev/e/select_multiple_invalid_value`);
}
function Pe() {
	console.warn(`https://svelte.dev/e/svelte_boundary_reset_noop`);
}
var T = !1;
function E(e) {
	T = e;
}
var D;
function O(e) {
	if (e === null) throw (Me(), Te);
	return (D = e);
}
function k() {
	return O(R(D));
}
function Fe(e) {
	if (T) {
		if (R(D) !== null) throw (Me(), Te);
		D = e;
	}
}
function Ie(e = 1) {
	if (T) {
		for (var t = e, n = D; t--; ) n = R(n);
		D = n;
	}
}
function Le(e = !0) {
	for (var t = 0, n = D; ; ) {
		if (n.nodeType === 8) {
			var r = n.data;
			if (r === `]`) {
				if (t === 0) return n;
				--t;
			} else
				(r === `[` || r === `[!` || (r[0] === `[` && !isNaN(Number(r.slice(1))))) &&
					(t += 1);
		}
		var i = R(n);
		(e && n.remove(), (n = i));
	}
}
function Re(e) {
	if (!e || e.nodeType !== 8) throw (Me(), Te);
	return e.data;
}
function ze(e) {
	return e === this.v;
}
function Be(e, t) {
	return e == e
		? e !== t || (typeof e == `object` && !!e) || typeof e == `function`
		: t == t;
}
function Ve(e) {
	return !Be(e, this.v);
}
function He(e) {
	throw Error(`https://svelte.dev/e/experimental_async_required`);
}
function Ue(e) {
	throw Error(`https://svelte.dev/e/lifecycle_outside_component`);
}
function We() {
	throw Error(`https://svelte.dev/e/missing_context`);
}
function Ge() {
	throw Error(`https://svelte.dev/e/async_derived_orphan`);
}
function Ke(e, t, n) {
	throw Error(`https://svelte.dev/e/each_key_duplicate`);
}
function qe(e) {
	throw Error(`https://svelte.dev/e/effect_in_teardown`);
}
function Je() {
	throw Error(`https://svelte.dev/e/effect_in_unowned_derived`);
}
function Ye(e) {
	throw Error(`https://svelte.dev/e/effect_orphan`);
}
function Xe() {
	throw Error(`https://svelte.dev/e/effect_update_depth_exceeded`);
}
function Ze() {
	throw Error(`https://svelte.dev/e/fork_discarded`);
}
function Qe() {
	throw Error(`https://svelte.dev/e/fork_timing`);
}
function $e() {
	throw Error(`https://svelte.dev/e/get_abort_signal_outside_reaction`);
}
function et() {
	throw Error(`https://svelte.dev/e/hydration_failed`);
}
function tt(e) {
	throw Error(`https://svelte.dev/e/lifecycle_legacy_only`);
}
function nt(e) {
	throw Error(`https://svelte.dev/e/props_invalid_value`);
}
function rt() {
	throw Error(`https://svelte.dev/e/state_descriptors_fixed`);
}
function it() {
	throw Error(`https://svelte.dev/e/state_prototype_fixed`);
}
function at() {
	throw Error(`https://svelte.dev/e/state_unsafe_mutation`);
}
function ot() {
	throw Error(`https://svelte.dev/e/svelte_boundary_reset_onerror`);
}
var st = !1;
function ct() {
	st = !0;
}
var lt = [];
function ut(e, t = !1, n = !1) {
	return dt(e, new Map(), ``, lt, null, n);
}
function dt(e, t, r, i, a = null, o = !1) {
	if (typeof e == `object` && e) {
		var s = t.get(e);
		if (s !== void 0) return s;
		if (e instanceof Map) return new Map(e);
		if (e instanceof Set) return new Set(e);
		if (n(e)) {
			var c = Array(e.length);
			(t.set(e, c), a !== null && t.set(a, c));
			for (var u = 0; u < e.length; u += 1) {
				var f = e[u];
				u in e && (c[u] = dt(f, t, r, i, null, o));
			}
			return c;
		}
		if (d(e) === l) {
			((c = {}), t.set(e, c), a !== null && t.set(a, c));
			for (var p of Object.keys(e)) c[p] = dt(e[p], t, r, i, null, o);
			return c;
		}
		if (e instanceof Date) return (e.getTime(), structuredClone(e));
		if (typeof e.toJSON == `function` && !o) return dt(e.toJSON(), t, r, i, e);
	}
	if (e instanceof EventTarget) return e;
	try {
		return structuredClone(e);
	} catch {
		return e;
	}
}
function ft(e, t) {
	return ((e.label = t), pt(e.v, t), e);
}
function pt(e, t) {
	return (e?.[_e]?.(t), e);
}
function mt(e) {
	return typeof e == `symbol`
		? `Symbol(${e.description})`
		: typeof e == `function`
			? `<function>`
			: typeof e == `object` && e
				? `<object>`
				: String(e);
}
function ht(e, t, n) {
	let r = {};
	return [() => (n(r) || We(), e(r)), (e) => t(r, e), () => n(r)];
}
function gt(e) {
	let t = e.p;
	for (; t !== null && t.c === null; ) t = t.p;
	return t?.c ?? null;
}
function _t(e, t) {
	return (e === null && Ue(t), (e.c ??= new Map(gt(e) || void 0)));
}
var A = null;
function vt(e) {
	A = e;
}
function yt() {
	return ht(bt, xt, St);
}
function bt(e) {
	return _t(A, `getContext`).get(e);
}
function xt(e, t) {
	return (_t(A, `setContext`).set(e, t), t);
}
function St(e) {
	return _t(A, `hasContext`).has(e);
}
function Ct() {
	return _t(A, `getAllContexts`);
}
function wt(e, t = !1, n) {
	A = {
		p: A,
		i: !1,
		c: null,
		e: null,
		s: e,
		x: null,
		r: K,
		l: st && !t ? { s: null, u: null, $: [] } : null
	};
}
function Tt(e) {
	var t = A,
		n = t.e;
	if (n !== null) {
		t.e = null;
		for (var r of n) ar(r);
	}
	return (e !== void 0 && (t.x = e), (t.i = !0), (A = t.p), Et(e));
}
function Et(e = {}) {
	return (o(e, me, { value: !0 }), e);
}
function Dt() {
	return !st || (A !== null && A.l === null);
}
var Ot = [];
function kt() {
	var e = Ot;
	((Ot = []), g(e));
}
function j(e) {
	if (Ot.length === 0 && !cn) {
		var t = Ot;
		queueMicrotask(() => {
			t === Ot && kt();
		});
	}
	Ot.push(e);
}
function At() {
	for (; Ot.length > 0; ) kt();
}
var jt = ~(S | C | x);
function M(e, t) {
	e.f = (e.f & jt) | t;
}
function Mt(e) {
	e.f & 512 || e.deps === null ? M(e, x) : M(e, C);
}
function Nt(e) {
	if (e !== null) for (let t of e) t.f & 2 && t.f & 65536 && ((t.f ^= le), Nt(t.deps));
}
function Pt(e, t, n) {
	(e.f & 2048 ? t.add(e) : e.f & 4096 && n.add(e), Nt(e.deps), M(e, x));
}
var Ft = [];
function It(e, t = m) {
	let n = null,
		r = new Set();
	function i(t) {
		if (Be(e, t) && ((e = t), n)) {
			let t = !Ft.length;
			for (let t of r) (t[1](), Ft.push(t, e));
			if (t) {
				for (let e = 0; e < Ft.length; e += 2) Ft[e][0](Ft[e + 1]);
				Ft.length = 0;
			}
		}
	}
	function a(t) {
		i(t(e));
	}
	function o(o, s = m) {
		let c = [o, s];
		return (
			r.add(c),
			r.size === 1 && (n = t(i, a) || m),
			o(e),
			() => {
				(r.delete(c), r.size === 0 && n && (n(), (n = null)));
			}
		);
	}
	return { set: i, update: a, subscribe: o };
}
var Lt = !1;
function Rt(e) {
	var t = Lt;
	try {
		return ((Lt = !1), [e(), Lt]);
	} finally {
		Lt = t;
	}
}
function zt(e, t) {
	if (t) {
		let t = document.body;
		((e.autofocus = !0),
			j(() => {
				document.activeElement === t && e.focus();
			}));
	}
}
var Bt = !1;
function Vt() {
	Bt ||
		((Bt = !0),
		document.addEventListener(
			`reset`,
			(e) => {
				Promise.resolve().then(() => {
					if (!e.defaultPrevented) for (let t of e.target.elements) t[Se]?.();
				});
			},
			{ capture: !0 }
		));
}
function Ht(e) {
	var t = U,
		n = K;
	(G(null), q(null));
	try {
		return e();
	} finally {
		(G(t), q(n));
	}
}
function Ut(e, t, n, r = n) {
	e.addEventListener(t, () => Ht(n));
	let i = e[Se];
	((e[Se] = i
		? () => {
				(i(), r(!0));
			}
		: () => r(!0)),
		Vt());
}
function Wt(e, t, n, r) {
	let i = Dt() ? Jt : Qt;
	var a = e.filter((e) => !e.settled),
		o = t.map(i);
	if (n.length === 0 && a.length === 0) {
		r(o);
		return;
	}
	var s = K,
		c = Gt(),
		l =
			a.length === 1
				? a[0].promise
				: a.length > 1
					? Promise.all(a.map((e) => e.promise))
					: null;
	function u(e) {
		if (!(s.f & 16384)) {
			c();
			try {
				r([...o, ...e]);
			} catch (e) {
				z(e, s);
			}
			Kt();
		}
	}
	var d = qt();
	if (n.length === 0) {
		l.then(() => u([])).finally(d);
		return;
	}
	function f() {
		Promise.all(n.map((e) => Xt(e)))
			.then(u)
			.catch((e) => z(e, s))
			.finally(d);
	}
	l
		? l.then(() => {
				(c(), f(), Kt());
			})
		: f();
}
function Gt() {
	var e = K,
		t = U,
		n = A,
		r = N;
	return function (i = !0) {
		(q(e), G(t), vt(n), i && !(e.f & 16384) && (r?.activate(), r?.apply()));
	};
}
function Kt(e = !0) {
	(q(null), G(null), vt(null), e && N?.deactivate());
}
function qt() {
	var e = K,
		t = e.b,
		n = N,
		r = !!t?.is_rendered();
	return (
		t?.update_pending_count(1, n),
		n.increment(r, e),
		() => {
			(t?.update_pending_count(-1, n), n.decrement(r, e));
		}
	);
}
function Jt(e) {
	var t = 2 | S;
	return (
		K !== null && (K.f |= oe),
		{
			ctx: A,
			deps: null,
			effects: null,
			equals: ze,
			f: t,
			fn: e,
			reactions: null,
			rv: 0,
			v: w,
			wv: 0,
			parent: K,
			ac: null
		}
	);
}
var Yt = Symbol(`obsolete`);
function Xt(e, t, n) {
	let r = K;
	r === null && Ge();
	var i = void 0,
		a = On(w),
		o = !U,
		s = new Set();
	return (
		ur(() => {
			var t = K,
				n = _();
			i = n.promise;
			try {
				Promise.resolve(e())
					.then(n.resolve, (e) => {
						e !== Ce && n.reject(e);
					})
					.finally(Kt);
			} catch (e) {
				(n.reject(e), Kt());
			}
			var c = N;
			if (o) {
				if (t.f & 32768) var l = qt();
				if (r.b?.is_rendered()) c.async_deriveds.get(t)?.reject(Yt);
				else for (let e of s.values()) e.reject(Yt);
				(s.add(n), c.async_deriveds.set(t, n));
			}
			let u = (e, t = void 0) => {
				(l?.(),
					s.delete(n),
					t !== Yt &&
						(c.activate(),
						t ? ((a.f |= fe), jn(a, t)) : (a.f & 8388608 && (a.f ^= fe), jn(a, e)),
						c.deactivate()));
			};
			n.promise.then(u, (e) => u(null, e || `unknown`));
		}),
		rr(() => {
			for (let e of s) e.reject(Yt);
		}),
		new Promise((e) => {
			function t(n) {
				function r() {
					n === i ? e(a) : t(i);
				}
				n.then(r, r);
			}
			t(i);
		})
	);
}
function Zt(e) {
	let t = Jt(e);
	return (Ar(t), t);
}
function Qt(e) {
	let t = Jt(e);
	return ((t.equals = Ve), t);
}
function $t(e) {
	var t = e.effects;
	if (t !== null) {
		e.effects = null;
		for (var n = 0; n < t.length; n += 1) H(t[n]);
	}
}
function en(e) {
	var t,
		n = K,
		r = e.parent;
	if (!Or && r !== null && e.v !== w && r.f & 24576) return (Ae(), e.v);
	q(r);
	try {
		((e.f &= ~le), $t(e), (t = zr(e)));
	} finally {
		q(n);
	}
	return t;
}
function tn(e) {
	var t = en(e);
	if (
		!e.equals(t) &&
		((e.wv = Ir()),
		(!N?.is_fork || e.deps === null) &&
			(N === null ? (e.v = t) : (N.capture(e, t, !0), on?.capture(e, t, !0)),
			e.deps === null))
	) {
		M(e, x);
		return;
	}
	Or || (P === null ? Mt(e) : (nr() || N?.is_fork) && P.set(e, t));
}
function nn(e) {
	if (e.effects !== null)
		for (let t of e.effects)
			(t.teardown || t.ac) &&
				(t.teardown?.(),
				t.ac !== null &&
					Ht(() => {
						(t.ac.abort(Ce), (t.ac = null));
					}),
				t.fn !== null && (t.teardown = m),
				Hr(t, 0),
				_r(t));
}
function rn(e) {
	if (e.effects !== null) for (let t of e.effects) t.teardown && t.fn !== null && Ur(t);
}
var an = null,
	N = null,
	on = null,
	P = null,
	sn = null,
	cn = !1,
	ln = !1,
	un = null,
	dn = null,
	fn = 0,
	pn = 1,
	mn = class e {
		id = pn++;
		#e = !1;
		linked = !0;
		#t = null;
		#n = null;
		async_deriveds = new Map();
		current = new Map();
		previous = new Map();
		#r = new Set();
		#i = new Set();
		#a = 0;
		#o = new Map();
		#s = null;
		#c = [];
		#l = [];
		#u = new Set();
		#d = new Set();
		#f = new Map();
		#p = new Set();
		is_fork = !1;
		#m = !1;
		constructor() {
			(an === null ? (an = this) : ((an.#n = this), (this.#t = an)), (an = this));
		}
		#h() {
			if (this.is_fork) return !0;
			for (let n of this.#o.keys()) {
				for (var e = n, t = !1; e.parent !== null; ) {
					if (this.#f.has(e)) {
						t = !0;
						break;
					}
					e = e.parent;
				}
				if (!t) return !0;
			}
			return !1;
		}
		skip_effect(e) {
			(this.#f.has(e) || this.#f.set(e, { d: [], m: [] }), this.#p.delete(e));
		}
		unskip_effect(e, t = (e) => this.schedule(e)) {
			var n = this.#f.get(e);
			if (n) {
				this.#f.delete(e);
				for (var r of n.d) (M(r, S), t(r));
				for (r of n.m) (M(r, C), t(r));
			}
			this.#p.add(e);
		}
		#g() {
			((this.#e = !0), fn++ > 1e3 && (this.#x(), gn()));
			for (let e of this.#u) (this.#d.delete(e), M(e, S), this.schedule(e));
			for (let e of this.#d) (M(e, C), this.schedule(e));
			let t = this.#c;
			((this.#c = []), this.apply());
			var n = (un = []),
				r = [],
				i = (dn = []);
			for (let e of t)
				try {
					this.#_(e, n, r);
				} catch (t) {
					throw (Sn(e), this.#h() || this.discard(), t);
				}
			if (((N = null), i.length > 0)) {
				var a = e.ensure();
				for (let e of i) a.schedule(e);
			}
			if (((un = null), (dn = null), this.#h())) {
				(this.#b(r), this.#b(n));
				for (let [e, t] of this.#f) xn(e, t);
				i.length > 0 && N.#g();
				return;
			}
			let o = this.#v();
			if (o) {
				(this.#b(r), this.#b(n), o.#y(this));
				return;
			}
			(this.#u.clear(), this.#d.clear());
			for (let e of this.#r) e(this);
			(this.#r.clear(), (on = this), vn(r), vn(n), (on = null), this.#s?.resolve());
			var s = N;
			if (
				(this.#a === 0 && (this.#c.length === 0 || s !== null) && this.#x(),
				this.#c.length > 0)
			) {
				if (s !== null) {
					let e = s;
					e.#c.push(...this.#c.filter((t) => !e.#c.includes(t)));
				} else s = this;
			}
			s !== null && (Tn.clear(), s.#g());
		}
		#_(e, t, n) {
			e.f ^= x;
			for (var r = e.first; r !== null; ) {
				var i = r.f,
					a = !!(i & 96);
				if (!((a && i & 1024) || i & 8192 || this.#f.has(r)) && r.fn !== null) {
					a ? (r.f ^= x) : i & 4 ? t.push(r) : Lr(r) && (i & 16 && this.#d.add(r), Ur(r));
					var o = r.first;
					if (o !== null) {
						r = o;
						continue;
					}
				}
				for (; r !== null; ) {
					var s = r.next;
					if (s !== null) {
						r = s;
						break;
					}
					r = r.parent;
				}
			}
		}
		#v() {
			for (var e = this.#t; e !== null; ) {
				if (!e.is_fork) {
					for (let [t, [, n]] of this.current) if (e.current.has(t) && !n) return e;
				}
				e = e.#t;
			}
			return null;
		}
		#y(e) {
			for (let [t, n] of e.current)
				(!this.previous.has(t) &&
					e.previous.has(t) &&
					this.previous.set(t, e.previous.get(t)),
					this.current.set(t, n));
			for (let [t, n] of e.async_deriveds) {
				let e = this.async_deriveds.get(t);
				e && n.promise.then(e.resolve).catch(e.reject);
			}
			(e.async_deriveds.clear(), this.transfer_effects(e.#u, e.#d));
			let t = (e) => {
				var n = e.reactions;
				if (n !== null && !(e.f & 2 && !(e.f & 6144)))
					for (let e of n) {
						var r = e.f;
						if (r & 2) t(e);
						else {
							var i = e;
							r & 4194320 &&
								!this.async_deriveds.has(i) &&
								(this.#d.delete(i), M(i, S), this.schedule(i));
						}
					}
			};
			for (let e of this.current.keys()) t(e);
			(this.oncommit(() => e.discard()), e.#x(), (N = this), this.#g());
		}
		#b(e) {
			for (var t = 0; t < e.length; t += 1) Pt(e[t], this.#u, this.#d);
		}
		capture(e, t, n = !1) {
			(e.v !== w && !this.previous.has(e) && this.previous.set(e, e.v),
				e.f & 8388608 || (this.current.set(e, [t, n]), P?.set(e, t)),
				this.is_fork || (e.v = t));
		}
		activate() {
			N = this;
		}
		deactivate() {
			((N = null), (P = null));
		}
		flush() {
			try {
				((ln = !0), (N = this), this.#g());
			} finally {
				((fn = 0),
					(sn = null),
					(un = null),
					(dn = null),
					(ln = !1),
					(N = null),
					(P = null),
					Tn.clear());
			}
		}
		discard() {
			for (let e of this.#i) e(this);
			this.#i.clear();
			for (let e of this.async_deriveds.values()) e.reject(Yt);
			(this.#x(), this.#s?.resolve());
		}
		register_created_effect(e) {
			this.#l.push(e);
		}
		increment(e, t) {
			if (((this.#a += 1), e)) {
				let e = this.#o.get(t) ?? 0;
				this.#o.set(t, e + 1);
			}
		}
		decrement(e, t) {
			if ((--this.#a, e)) {
				let e = this.#o.get(t) ?? 0;
				e === 1 ? this.#o.delete(t) : this.#o.set(t, e - 1);
			}
			this.#m ||
				((this.#m = !0),
				j(() => {
					((this.#m = !1), this.linked && this.flush());
				}));
		}
		transfer_effects(e, t) {
			for (let t of e) this.#u.add(t);
			for (let e of t) this.#d.add(e);
			(e.clear(), t.clear());
		}
		oncommit(e) {
			this.#r.add(e);
		}
		ondiscard(e) {
			this.#i.add(e);
		}
		settled() {
			return (this.#s ??= _()).promise;
		}
		static ensure() {
			if (N === null) {
				let t = (N = new e());
				!ln &&
					!cn &&
					j(() => {
						t.#e || t.flush();
					});
			}
			return N;
		}
		apply() {
			P = null;
		}
		schedule(e) {
			if (((sn = e), e.b?.is_pending && e.f & 16777228 && !(e.f & 32768))) {
				e.b.defer_effect(e);
				return;
			}
			for (var t = e; t.parent !== null; ) {
				t = t.parent;
				var n = t.f;
				if (un !== null && t === K && (U === null || !(U.f & 2))) return;
				if (n & 96) {
					if (!(n & 1024)) return;
					t.f ^= x;
				}
			}
			this.#c.push(t);
		}
		#x() {
			if (this.linked) {
				var e = this.#t,
					t = this.#n;
				(e === null || (e.#n = t),
					t === null ? (an = e) : (t.#t = e),
					(this.linked = !1));
			}
		}
	};
function hn(e) {
	var t = cn;
	cn = !0;
	try {
		var n;
		for (e && (N !== null && !N.is_fork && N.flush(), (n = e())); ; ) {
			if ((At(), N === null)) return n;
			N.flush();
		}
	} finally {
		cn = t;
	}
}
function gn() {
	try {
		Xe();
	} catch (e) {
		z(e, sn);
	}
}
var _n = null;
function vn(e) {
	var t = e.length;
	if (t !== 0) {
		for (var n = 0; n < t; ) {
			var r = e[n++];
			if (
				!(r.f & 24576) &&
				Lr(r) &&
				((_n = new Set()),
				Ur(r),
				r.deps === null &&
					r.first === null &&
					r.nodes === null &&
					r.teardown === null &&
					r.ac === null &&
					br(r),
				_n?.size > 0)
			) {
				Tn.clear();
				for (let e of _n) {
					if (e.f & 24576) continue;
					let t = [e],
						n = e.parent;
					for (; n !== null; ) (_n.has(n) && (_n.delete(n), t.push(n)), (n = n.parent));
					for (let e = t.length - 1; e >= 0; e--) {
						let n = t[e];
						n.f & 24576 || Ur(n);
					}
				}
				_n.clear();
			}
		}
		_n = null;
	}
}
function yn(e, t) {
	if (e.reactions !== null)
		for (let n of e.reactions) {
			let e = n.f;
			e & 2 ? yn(n, t) : e & 131072 && (M(n, S), t.add(n));
		}
}
function bn(e) {
	N.schedule(e);
}
function xn(e, t) {
	if (!(e.f & 32 && e.f & 1024)) {
		(e.f & 2048 ? t.d.push(e) : e.f & 4096 && t.m.push(e), M(e, x));
		for (var n = e.first; n !== null; ) (xn(n, t), (n = n.next));
	}
}
function Sn(e) {
	M(e, x);
	for (var t = e.first; t !== null; ) (Sn(t), (t = t.next));
}
function Cn(e) {
	(He(`fork`), N !== null && Qe());
	var t = mn.ensure();
	((t.is_fork = !0), (P = new Map()));
	var n = !1,
		r = t.settled();
	return (
		hn(e),
		{
			commit: async () => {
				if (n) {
					await r;
					return;
				}
				(t.linked || Ze(), (n = !0), (t.is_fork = !1));
				for (var [e, [i]] of t.current) ((e.v = i), (e.wv = Ir()));
				(hn(() => {
					var e = new Set();
					for (var n of t.current.keys()) yn(n, e);
					(En(e), Mn());
				}),
					t.flush(),
					await r);
			},
			discard: () => {
				for (var e of t.current.keys()) e.wv = Ir();
				!n && t.linked && t.discard();
			}
		}
	);
}
var wn = new Set(),
	Tn = new Map();
function En(e) {
	wn = e;
}
var Dn = !1;
function On(e, t) {
	return { f: 0, v: e, reactions: null, equals: ze, rv: 0, wv: 0 };
}
function kn(e, t) {
	let n = On(e, t);
	return (Ar(n), n);
}
function An(e, t = !1, n = !0) {
	let r = On(e);
	return (
		t || (r.equals = Ve),
		st && n && A !== null && A.l !== null && (A.l.s ??= []).push(r),
		r
	);
}
function F(e, t, n = !1) {
	return (
		U !== null &&
			(!W || U.f & 131072) &&
			Dt() &&
			U.f & 4325394 &&
			(J === null || !J.has(e)) &&
			at(),
		jn(e, n ? In(t) : t, dn)
	);
}
function jn(e, t, n = null) {
	if (!e.equals(t)) {
		Or ? Tn.set(e, t) : Tn.has(e) || Tn.set(e, e.v);
		var r = mn.ensure();
		if ((r.capture(e, t), e.f & 2)) {
			let t = e;
			(e.f & 2048 && en(t), P === null && Mt(t));
		}
		((e.wv = Ir()),
			Fn(e, S, n),
			Dt() &&
				K !== null &&
				K.f & 1024 &&
				!(K.f & 96) &&
				(Z === null ? jr([e]) : Z.push(e)),
			!r.is_fork && wn.size > 0 && !Dn && Mn());
	}
	return t;
}
function Mn() {
	Dn = !1;
	for (let e of wn) {
		e.f & 1024 && M(e, C);
		let t;
		try {
			t = Lr(e);
		} catch {
			t = !0;
		}
		t && Ur(e);
	}
	wn.clear();
}
function Nn(e, t = 1) {
	var n = Q(e),
		r = t === 1 ? n++ : n--;
	return (F(e, n), r);
}
function Pn(e) {
	F(e, e.v + 1);
}
function Fn(e, t, n) {
	var r = e.reactions;
	if (r !== null)
		for (var i = Dt(), a = r.length, o = 0; o < a; o++) {
			var s = r[o],
				c = s.f;
			if (i || s !== K) {
				var l = (c & S) === 0;
				if ((l && M(s, t), c & 131072)) wn.add(s);
				else if (c & 2) {
					var u = s;
					(P?.delete(u),
						c & 65536 ||
							(c & 512 && (K === null || !(K.f & 2097152)) && (s.f |= le), Fn(u, C, n)));
				} else if (l) {
					var d = s;
					(c & 16 && _n !== null && _n.add(d), n === null ? bn(d) : n.push(d));
				}
			}
		}
}
function In(e) {
	if (typeof e != `object` || !e || pe in e || me in e) return e;
	let t = d(e);
	if (t !== l && t !== u) return e;
	var r = new Map(),
		i = n(e),
		a = kn(0),
		o = null,
		c = Pr,
		f = (e) => {
			if (Pr === c) return e();
			var t = U,
				n = Pr;
			(G(null), Fr(c));
			var r = e();
			return (G(t), Fr(n), r);
		};
	return (
		i && r.set(`length`, kn(e.length, o)),
		new Proxy(e, {
			defineProperty(e, t, n) {
				(!(`value` in n) ||
					n.configurable === !1 ||
					n.enumerable === !1 ||
					n.writable === !1) &&
					rt();
				var i = r.get(t);
				return (
					i === void 0
						? f(() => {
								var e = kn(n.value, o);
								return (r.set(t, e), e);
							})
						: F(i, n.value, !0),
					!0
				);
			},
			deleteProperty(e, t) {
				var n = r.get(t);
				if (n === void 0) {
					if (t in e) {
						let e = f(() => kn(w, o));
						(r.set(t, e), Pn(a));
					}
				} else (F(n, w), Pn(a));
				return !0;
			},
			get(t, n, i) {
				if (n === pe) return e;
				var a = r.get(n),
					c = n in t;
				if (
					(a === void 0 &&
						(!c || s(t, n)?.writable) &&
						((a = f(() => kn(In(c ? t[n] : w), o))), r.set(n, a)),
					a !== void 0)
				) {
					var l = Q(a);
					return l === w ? void 0 : l;
				}
				return Reflect.get(t, n, i);
			},
			getOwnPropertyDescriptor(e, t) {
				var n = Reflect.getOwnPropertyDescriptor(e, t);
				if (n && `value` in n) {
					var i = r.get(t);
					i && (n.value = Q(i));
				} else if (n === void 0) {
					var a = r.get(t),
						o = a?.v;
					if (a !== void 0 && o !== w)
						return { enumerable: !0, configurable: !0, value: o, writable: !0 };
				}
				return n;
			},
			has(e, t) {
				if (t === pe) return !0;
				var n = r.get(t),
					i = (n !== void 0 && n.v !== w) || Reflect.has(e, t);
				return (n !== void 0 || (K !== null && (!i || s(e, t)?.writable))) &&
					(n === void 0 && ((n = f(() => kn(i ? In(e[t]) : w, o))), r.set(t, n)),
					Q(n) === w)
					? !1
					: i;
			},
			set(e, t, n, c) {
				var l = r.get(t),
					u = t in e;
				if (i && t === `length`)
					for (var d = n; d < l.v; d += 1) {
						var p = r.get(d + ``);
						p === void 0
							? d in e && ((p = f(() => kn(w, o))), r.set(d + ``, p))
							: F(p, w);
					}
				if (l === void 0)
					(!u || s(e, t)?.writable) &&
						((l = f(() => kn(void 0, o))), F(l, In(n)), r.set(t, l));
				else {
					u = l.v !== w;
					var m = f(() => In(n));
					F(l, m);
				}
				var h = Reflect.getOwnPropertyDescriptor(e, t);
				if ((h?.set && h.set.call(c, n), !u)) {
					if (i && typeof t == `string`) {
						var g = r.get(`length`),
							_ = Number(t);
						Number.isInteger(_) && _ >= g.v && F(g, _ + 1);
					}
					Pn(a);
				}
				return !0;
			},
			ownKeys(e) {
				Q(a);
				var t = Reflect.ownKeys(e).filter((e) => {
					var t = r.get(e);
					return t === void 0 || t.v !== w;
				});
				for (var [n, i] of r) i.v !== w && !(n in e) && t.push(n);
				return t;
			},
			setPrototypeOf() {
				it();
			}
		})
	);
}
function Ln(e) {
	try {
		if (typeof e == `object` && e && pe in e) return e[pe];
	} catch {}
	return e;
}
function Rn(e, t) {
	return Object.is(Ln(e), Ln(t));
}
var zn, Bn, Vn, Hn, Un;
function Wn() {
	if (zn === void 0) {
		((zn = window), (Bn = document), (Vn = /Firefox/.test(navigator.userAgent)));
		var e = Element.prototype,
			t = Node.prototype,
			n = Text.prototype;
		((Hn = s(t, `firstChild`).get),
			(Un = s(t, `nextSibling`).get),
			f(e) && ((e[ye] = void 0), (e[ve] = null), (e[be] = void 0), (e.__e = void 0)),
			f(n) && (n[xe] = void 0));
	}
}
function I(e = ``) {
	return document.createTextNode(e);
}
function L(e) {
	return Hn.call(e);
}
function R(e) {
	return Un.call(e);
}
function Gn(e, t) {
	if (!T) return L(e);
	var n = L(D);
	if (n === null) n = D.appendChild(I());
	else if (t && n.nodeType !== 3) {
		var r = I();
		return (n?.before(r), O(r), r);
	}
	return (t && Qn(n), O(n), n);
}
function Kn(e, t = !1) {
	if (!T) {
		var n = L(e);
		return n instanceof Comment && n.data === `` ? R(n) : n;
	}
	if (t) {
		if (D?.nodeType !== 3) {
			var r = I();
			return (D?.before(r), O(r), r);
		}
		Qn(D);
	}
	return D;
}
function qn(e, t = !1) {
	if (!T) return L(e);
	var n = Gn(e, t);
	return (Fe(e), n);
}
function Jn(e, t = 1, n = !1) {
	let r = T ? D : e;
	for (var i; t--; ) ((i = r), (r = R(r)));
	if (!T) return r;
	if (n) {
		if (r?.nodeType !== 3) {
			var a = I();
			return (r === null ? i?.after(a) : r.before(a), O(a), a);
		}
		Qn(r);
	}
	return (O(r), r);
}
function Yn(e) {
	e.textContent = ``;
}
function Xn() {
	return !1;
}
function Zn(e, t, n) {
	return t == null || t === `http://www.w3.org/1999/xhtml`
		? n
			? document.createElement(e, { is: n })
			: document.createElement(e)
		: n
			? document.createElementNS(t, e, { is: n })
			: document.createElementNS(t, e);
}
function Qn(e) {
	if (e.nodeValue.length < 65536) return;
	let t = e.nextSibling;
	for (; t !== null && t.nodeType === 3; )
		(t.remove(), (e.nodeValue += t.nodeValue), (t = e.nextSibling));
}
function $n(e) {
	var t = K;
	if (t === null) return ((U.f |= fe), e);
	if (!(t.f & 32768) && !(t.f & 4)) throw e;
	z(e, t);
}
function z(e, t) {
	if (!(t !== null && t.f & 16384)) {
		for (; t !== null; ) {
			if (t.f & 128 && !(t.f & 33570816)) {
				if (!(t.f & 32768)) throw e;
				try {
					t.b.error(e);
					return;
				} catch (t) {
					e = t;
				}
			}
			t = t.parent;
		}
		throw e;
	}
}
function er(e) {
	(K === null && (U === null && Ye(e), Je()), Or && qe(e));
}
function tr(e, t) {
	var n = t.last;
	n === null ? (t.last = t.first = e) : ((n.next = e), (e.prev = n), (t.last = e));
}
function B(e, t) {
	var n = K;
	n !== null && n.f & 8192 && (e |= ee);
	var r = {
		ctx: A,
		deps: null,
		nodes: null,
		f: e | S | 512,
		first: null,
		fn: t,
		last: null,
		next: null,
		parent: n,
		b: n && n.b,
		prev: null,
		teardown: null,
		wv: 0,
		ac: null
	};
	N?.register_created_effect(r);
	var i = r;
	if (e & 4) un === null ? mn.ensure().schedule(r) : un.push(r);
	else if (t !== null) {
		try {
			Ur(r);
		} catch (e) {
			throw (H(r), e);
		}
		i.deps === null &&
			i.teardown === null &&
			i.nodes === null &&
			i.first === i.last &&
			!(i.f & 524288) &&
			((i = i.first), e & 16 && e & 65536 && i !== null && (i.f |= ie));
	}
	if (
		i !== null &&
		((i.parent = n), n !== null && tr(i, n), U !== null && U.f & 2 && !(e & 64))
	) {
		var a = U;
		(a.effects ??= []).push(i);
	}
	return r;
}
function nr() {
	return U !== null && !W;
}
function rr(e) {
	let t = B(8, null);
	return (M(t, x), (t.teardown = e), t);
}
function ir(e) {
	er(`$effect`);
	var t = K.f;
	if (!U && t & 32 && A !== null && !A.i) {
		var n = A;
		(n.e ??= []).push(e);
	} else return ar(e);
}
function ar(e) {
	return B(4 | se, e);
}
function or(e) {
	return (er(`$effect.pre`), B(8 | se, e));
}
function sr(e) {
	mn.ensure();
	let t = B(64 | oe, e);
	return () => {
		H(t);
	};
}
function cr(e) {
	mn.ensure();
	let t = B(64 | oe, e);
	return (e = {}) =>
		new Promise((n) => {
			e.outro
				? xr(t, () => {
						(H(t), n(void 0));
					})
				: (H(t), n(void 0));
		});
}
function lr(e) {
	return B(4, e);
}
function ur(e) {
	return B(de | oe, e);
}
function dr(e, t = 0) {
	return B(8 | t, e);
}
function fr(e, t = [], n = [], r = []) {
	Wt(r, t, n, (t) => {
		B(8, () => {
			e(...t.map(Q));
		});
	});
}
function pr(e, t = [], n = [], r = []) {
	Wt(r, t, n, (t) => {
		B(4, () => e(...t.map(Q)));
	});
}
function mr(e, t = 0) {
	return B(16 | t, e);
}
function hr(e, t = 0) {
	return B(b | t, e);
}
function V(e) {
	return B(32 | oe, e);
}
function gr(e) {
	var t = e.teardown;
	if (t !== null) {
		let n = Or,
			r = U;
		(kr(!0), G(null));
		try {
			t.call(null);
		} catch (t) {
			z(t, e.parent);
		} finally {
			(kr(n), G(r));
		}
	}
}
function _r(e, t = !1) {
	var n = e.first;
	for (e.first = e.last = null; n !== null; ) {
		let e = n.ac;
		e !== null &&
			Ht(() => {
				e.abort(Ce);
			});
		var r = n.next;
		(n.f & 64 ? (n.parent = null) : H(n, t), (n = r));
	}
}
function vr(e) {
	for (var t = e.first; t !== null; ) {
		var n = t.next;
		(t.f & 32 || H(t), (t = n));
	}
}
function H(e, t = !0) {
	var n = !1;
	((t || e.f & 262144) &&
		e.nodes !== null &&
		e.nodes.end !== null &&
		(yr(e.nodes.start, e.nodes.end), (n = !0)),
		(e.f |= re),
		_r(e, t && !n),
		Hr(e, 0));
	var r = e.nodes && e.nodes.t;
	if (r !== null) for (let e of r) e.stop();
	(gr(e), (e.f ^= re), (e.f |= te));
	var i = e.parent;
	(i !== null && i.first !== null && br(e),
		(e.next = e.prev = e.teardown = e.ctx = e.deps = e.fn = e.nodes = e.ac = e.b = null));
}
function yr(e, t) {
	for (; e !== null; ) {
		var n = e === t ? null : R(e);
		(e.remove(), (e = n));
	}
}
function br(e) {
	var t = e.parent,
		n = e.prev,
		r = e.next;
	(n !== null && (n.next = r),
		r !== null && (r.prev = n),
		t !== null && (t.first === e && (t.first = r), t.last === e && (t.last = n)));
}
function xr(e, t, n = !0) {
	var r = [];
	((e.f |= 256), Sr(e, r, !0));
	var i = () => {
			(n && H(e), t && t());
		},
		a = r.length;
	if (a > 0) {
		var o = () => --a || i();
		for (var s of r) s.out(o);
	} else i();
}
function Sr(e, t, n) {
	if (!(e.f & 8192)) {
		e.f ^= ee;
		var r = e.nodes && e.nodes.t;
		if (r !== null) for (let e of r) (e.is_global || n) && t.push(e);
		for (var i = e.first; i !== null; ) {
			var a = i.next;
			if (!(i.f & 64)) {
				var o = !!(i.f & 65536) || (!!(i.f & 32) && !!(e.f & 16));
				Sr(i, t, o ? n : !1);
			}
			i = a;
		}
	}
}
function Cr(e) {
	((e.f &= -257), wr(e, !0));
}
function wr(e, t) {
	if (!(e.f & 256) && e.f & 8192) {
		((e.f ^= ee), e.f & 1024 || (M(e, S), mn.ensure().schedule(e)));
		for (var n = e.first; n !== null; ) {
			var r = n.next,
				i = !!(n.f & 65536) || !!(n.f & 32);
			(wr(n, i ? t : !1), (n = r));
		}
		var a = e.nodes && e.nodes.t;
		if (a !== null) for (let e of a) (e.is_global || t) && e.in();
	}
}
function Tr(e, t) {
	if (e.nodes)
		for (var n = e.nodes.start, r = e.nodes.end; n !== null; ) {
			var i = n === r ? null : R(n);
			(t.append(n), (n = i));
		}
}
var Er = null,
	Dr = !1,
	Or = !1;
function kr(e) {
	Or = e;
}
var U = null,
	W = !1;
function G(e) {
	U = e;
}
var K = null;
function q(e) {
	K = e;
}
var J = null;
function Ar(e) {
	U !== null && (J ??= new Set()).add(e);
}
var Y = null,
	X = 0,
	Z = null;
function jr(e) {
	Z = e;
}
var Mr = 1,
	Nr = 0,
	Pr = Nr;
function Fr(e) {
	Pr = e;
}
function Ir() {
	return ++Mr;
}
function Lr(e) {
	var t = e.f;
	if (t & 2048) return !0;
	if ((t & 2 && (e.f &= ~le), t & 4096)) {
		for (var n = e.deps, r = n.length, i = 0; i < r; i++) {
			var a = n[i];
			if ((Lr(a) && tn(a), a.wv > e.wv)) return !0;
		}
		t & 512 && P === null && M(e, x);
	}
	return !1;
}
function Rr(e, t, n = !0) {
	var r = e.reactions;
	if (r !== null && !(J !== null && J.has(e)))
		for (var i = 0; i < r.length; i++) {
			var a = r[i];
			a.f & 2 ? Rr(a, t, !1) : t === a && (n ? M(a, S) : a.f & 1024 && M(a, C), bn(a));
		}
}
function zr(e) {
	var t = Y,
		n = X,
		r = Z,
		i = U,
		a = J,
		o = A,
		s = W,
		c = Pr,
		l = e.f;
	((Y = null),
		(X = 0),
		(Z = null),
		(U = l & 96 ? null : e),
		(J = null),
		vt(e.ctx),
		(W = !1),
		(Pr = ++Nr),
		e.ac !== null &&
			(Ht(() => {
				e.ac.abort(Ce);
			}),
			(e.ac = null)));
	try {
		e.f |= ue;
		var u = e.fn,
			d = u();
		e.f |= ne;
		var f = Br(e);
		if (Dt() && Z !== null && !W && f !== null && !(e.f & 6146))
			for (var p = 0; p < Z.length; p++) Rr(Z[p], e);
		if (i !== null && i !== e) {
			if ((Nr++, i.deps !== null)) for (let e = 0; e < n; e += 1) i.deps[e].rv = Nr;
			if (t !== null) for (let e of t) e.rv = Nr;
			Z !== null && (r === null ? (r = Z) : r.push(...Z));
		}
		return (e.f & 8388608 && (e.f ^= fe), d);
	} catch (t) {
		return (Br(e), $n(t));
	} finally {
		((e.f ^= ue), (Y = t), (X = n), (Z = r), (U = i), (J = a), vt(o), (W = s), (Pr = c));
	}
}
function Br(e) {
	var t = e.deps,
		n = N?.is_fork;
	if (Y !== null) {
		var r;
		if ((n || Hr(e, X), t !== null && X > 0))
			for (t.length = X + Y.length, r = 0; r < Y.length; r++) t[X + r] = Y[r];
		else e.deps = t = Y;
		if (nr() && e.f & 512) for (r = X; r < t.length; r++) (t[r].reactions ??= []).push(e);
	} else !n && t !== null && X < t.length && (Hr(e, X), (t.length = X));
	return t;
}
function Vr(e, t) {
	let n = t.reactions;
	if (n !== null) {
		var a = r.call(n, e);
		if (a !== -1) {
			var o = n.length - 1;
			o === 0 ? (n = t.reactions = null) : ((n[a] = n[o]), n.pop());
		}
	}
	if (n === null && t.f & 2 && (Y === null || !i.call(Y, t))) {
		var s = t;
		(s.f & 512 && ((s.f ^= 512), (s.f &= ~le)),
			s.v !== w && Mt(s),
			s.ac !== null &&
				Ht(() => {
					(s.ac.abort(Ce), (s.ac = null), M(s, S));
				}),
			nn(s),
			Hr(s, 0));
	}
}
function Hr(e, t) {
	var n = e.deps;
	if (n !== null) for (var r = t; r < n.length; r++) Vr(e, n[r]);
}
function Ur(e) {
	var t = e.f;
	if (!(t & 16384)) {
		M(e, x);
		var n = K,
			r = Dr;
		((K = e), (Dr = !(t & 96)));
		try {
			(t & 16777232 ? vr(e) : _r(e), gr(e));
			var i = zr(e);
			((e.teardown = typeof i == `function` ? i : null), (e.wv = Mr));
		} finally {
			((Dr = r), (K = n));
		}
	}
}
async function Wr() {
	(await Promise.resolve(), hn());
}
function Gr() {
	return mn.ensure().settled();
}
function Q(e) {
	var t = !!(e.f & 2);
	if (
		(Er?.add(e),
		U !== null && !W && !(K !== null && K.f & 16384) && (J === null || !J.has(e)))
	) {
		var n = U.deps;
		if (U.f & 2097152)
			e.rv < Nr &&
				((e.rv = Nr),
				Y === null && n !== null && n[X] === e
					? X++
					: Y === null
						? (Y = [e])
						: Y.push(e));
		else {
			((U.deps ??= []), i.call(U.deps, e) || U.deps.push(e));
			var r = e.reactions;
			r === null ? (e.reactions = [U]) : i.call(r, U) || r.push(U);
		}
	}
	if (Or && Tn.has(e)) return Tn.get(e);
	if (t) {
		var a = e;
		if (Or) {
			var o = a.v;
			return (
				((!(a.f & 1024) && a.reactions !== null) || qr(a)) && (o = en(a)),
				Tn.set(a, o),
				o
			);
		}
		var s = !(a.f & 512) && !W && U !== null && (Dr || !!(U.f & 512)),
			c = (a.f & ne) === 0;
		(Lr(a) && (s && (a.f |= 512), tn(a)), s && !c && (rn(a), Kr(a)));
	}
	if (P?.has(e)) return P.get(e);
	if (e.f & 8388608) throw e.v;
	return e.v;
}
function Kr(e) {
	if (((e.f |= 512), e.deps !== null))
		for (let t of e.deps)
			((t.reactions ??= []).push(e), t.f & 2 && !(t.f & 512) && (rn(t), Kr(t)));
}
function qr(e) {
	if (e.v === w) return !0;
	if (e.deps === null) return !1;
	for (let t of e.deps) if (Tn.has(t) || (t.f & 2 && qr(t))) return !0;
	return !1;
}
function Jr(e) {
	var t = W;
	try {
		return ((W = !0), e());
	} finally {
		W = t;
	}
}
function Yr(e) {
	if (!(typeof e != `object` || !e || e instanceof EventTarget)) {
		if (pe in e) Xr(e);
		else if (!Array.isArray(e))
			for (let t in e) {
				let n = e[t];
				typeof n == `object` && n && pe in n && Xr(n);
			}
	}
}
function Xr(e, t = new Set()) {
	if (typeof e == `object` && e && !(e instanceof EventTarget) && !t.has(e)) {
		(t.add(e), e instanceof Date && e.getTime());
		for (let n in e)
			try {
				Xr(e[n], t);
			} catch {}
		let n = d(e);
		if (
			n !== Object.prototype &&
			n !== Array.prototype &&
			n !== Map.prototype &&
			n !== Set.prototype &&
			n !== Date.prototype
		) {
			let t = c(n);
			for (let n in t) {
				let r = t[n].get;
				if (r)
					try {
						r.call(e);
					} catch {}
			}
		}
	}
}
function Zr() {
	return Symbol(ke);
}
function Qr(e) {
	return e.endsWith(`capture`) && e !== `gotpointercapture` && e !== `lostpointercapture`;
}
var $r = [
	`beforeinput`,
	`click`,
	`change`,
	`dblclick`,
	`contextmenu`,
	`focusin`,
	`focusout`,
	`input`,
	`keydown`,
	`keyup`,
	`mousedown`,
	`mousemove`,
	`mouseout`,
	`mouseover`,
	`mouseup`,
	`pointerdown`,
	`pointermove`,
	`pointerout`,
	`pointerover`,
	`pointerup`,
	`touchend`,
	`touchmove`,
	`touchstart`
];
function ei(e) {
	return $r.includes(e);
}
var ti =
		`allowfullscreen.async.autofocus.autoplay.checked.controls.default.disabled.formnovalidate.indeterminate.inert.ismap.loop.multiple.muted.nomodule.novalidate.open.playsinline.readonly.required.reversed.seamless.selected.webkitdirectory.defer.disablepictureinpicture.disableremoteplayback`.split(
			`.`
		),
	ni = {
		formnovalidate: `formNoValidate`,
		ismap: `isMap`,
		nomodule: `noModule`,
		playsinline: `playsInline`,
		readonly: `readOnly`,
		defaultvalue: `defaultValue`,
		defaultchecked: `defaultChecked`,
		srcobject: `srcObject`,
		novalidate: `noValidate`,
		allowfullscreen: `allowFullscreen`,
		disablepictureinpicture: `disablePictureInPicture`,
		disableremoteplayback: `disableRemotePlayback`
	};
function ri(e) {
	return ((e = e.toLowerCase()), ni[e] ?? e);
}
[...ti];
var ii = [`touchstart`, `touchmove`];
function ai(e) {
	return ii.includes(e);
}
var oi = [`textarea`, `script`, `style`, `title`];
function si(e) {
	return oi.includes(e);
}
var ci = Symbol(`events`),
	li = new Set(),
	ui = new Set();
function di(e) {
	if (!T) return;
	(e.removeAttribute(`onload`), e.removeAttribute(`onerror`));
	let t = e.__e;
	t !== void 0 &&
		((e.__e = void 0),
		queueMicrotask(() => {
			e.isConnected && e.dispatchEvent(t);
		}));
}
function fi(e, t, n, r = {}) {
	function i(e) {
		if ((r.capture || yi.call(t, e), !e.cancelBubble)) return Ht(() => n?.call(this, e));
	}
	return (
		e.startsWith(`pointer`) || e.startsWith(`touch`) || e === `wheel`
			? j(() => {
					t.addEventListener(e, i, r);
				})
			: t.addEventListener(e, i, r),
		i
	);
}
function pi(e, t, n, r = {}) {
	var i = fi(t, e, n, r);
	return () => {
		e.removeEventListener(t, i, r);
	};
}
function mi(e, t, n, r, i) {
	var a = { capture: r, passive: i },
		o = fi(e, t, n, a);
	(t === document.body ||
		t === window ||
		t === document ||
		t instanceof HTMLMediaElement) &&
		rr(() => {
			t.removeEventListener(e, o, a);
		});
}
function hi(e, t, n) {
	(t[ci] ??= {})[e] = n;
}
function gi(e) {
	for (var t = 0; t < e.length; t++) li.add(e[t]);
	for (var n of ui) n(e);
}
var _i = null,
	vi = !1;
function yi(e) {
	var t = this,
		n = t.ownerDocument,
		r = e.type,
		i = e.composedPath?.() || [],
		a = i[0] || e.target;
	((_i = e),
		vi ||
			((vi = !0),
			setTimeout(() => {
				((vi = !1), (_i = null));
			})));
	var s = 0,
		c = _i === e && e[ci];
	if (c) {
		var l = i.indexOf(c);
		if (l !== -1 && (t === document || t === window)) {
			e[ci] = t;
			return;
		}
		var u = i.indexOf(t);
		if (u === -1) return;
		l <= u && (s = l);
	}
	if (((a = i[s] || e.target), a !== t)) {
		o(e, `currentTarget`, {
			configurable: !0,
			get() {
				return a || n;
			}
		});
		var d = U,
			f = K;
		(G(null), q(null));
		try {
			for (var p, m = []; a !== null && a !== t; ) {
				try {
					var h = a[ci]?.[r];
					h != null && (!a.disabled || e.target === a) && h.call(a, e);
				} catch (e) {
					p ? m.push(e) : (p = e);
				}
				if (e.cancelBubble) break;
				(s++, (a = s < i.length ? i[s] : null));
			}
			if (p) {
				for (let e of m)
					queueMicrotask(() => {
						throw e;
					});
				throw p;
			}
		} finally {
			((e[ci] = t), delete e.currentTarget, G(d), q(f));
		}
	}
}
var bi =
	globalThis?.window?.trustedTypes &&
	globalThis.window.trustedTypes.createPolicy(`svelte-trusted-html`, {
		createHTML: (e) => e
	});
function xi(e) {
	return bi?.createHTML(e) ?? e;
}
function Si(e) {
	var t = Zn(`template`);
	return ((t.innerHTML = xi(e.replaceAll(`<!>`, `<!---->`))), t.content);
}
function $(e, t) {
	var n = K;
	n.nodes === null && (n.nodes = { start: e, end: t, a: null, t: null });
}
function Ci(e, t) {
	var n = !!(t & 1),
		r = !!(t & 2),
		i,
		a = !e.startsWith(`<!>`);
	return () => {
		if (T) return ($(D, null), D);
		i === void 0 && ((i = Si(a ? e : `<!>` + e)), n || (i = L(i)));
		var t = r || Vn ? document.importNode(i, !0) : i.cloneNode(!0);
		if (n) {
			var o = L(t),
				s = t.lastChild;
			$(o, s);
		} else $(t, t);
		return t;
	};
}
function wi(e, t, n = `svg`) {
	var r = !e.startsWith(`<!>`),
		i = !!(t & 1),
		a = `<${n}>${r ? e : `<!>` + e}</${n}>`,
		o;
	return () => {
		if (T) return ($(D, null), D);
		if (!o) {
			var e = L(Si(a));
			if (i) for (o = document.createDocumentFragment(); L(e); ) o.appendChild(L(e));
			else o = L(e);
		}
		var t = o.cloneNode(!0);
		if (i) {
			var n = L(t),
				r = t.lastChild;
			$(n, r);
		} else $(t, t);
		return t;
	};
}
function Ti(e, t) {
	return wi(e, t, `svg`);
}
function Ei(e = ``) {
	if (!T) {
		var t = I(e + ``);
		return ($(t, t), t);
	}
	var n = D;
	return (n.nodeType === 3 ? Qn(n) : (n.before((n = I())), O(n)), $(n, n), n);
}
function Di() {
	if (T) return ($(D, null), D);
	var e = document.createDocumentFragment(),
		t = document.createComment(``),
		n = I();
	return (e.append(t, n), $(t, n), e);
}
function Oi(e, t) {
	if (T) {
		var n = K;
		((!(n.f & 32768) || n.nodes.end === null) && (n.nodes.end = D), k());
		return;
	}
	e !== null && e.before(t);
}
function ki() {
	if (T && D && D.nodeType === 8 && D.textContent?.startsWith(`$`)) {
		let e = D.textContent.substring(1);
		return (k(), e);
	}
	return (((window.__svelte ??= {}).uid ??= 1), `c${window.__svelte.uid++}`);
}
function Ai(e) {
	let t = 0,
		n = On(0),
		r;
	return () => {
		nr() &&
			(Q(n),
			dr(
				() => (
					t === 0 && (r = Jr(() => e(() => Pn(n)))),
					(t += 1),
					() => {
						j(() => {
							(--t, t === 0 && (r?.(), (r = void 0), Pn(n)));
						});
					}
				)
			));
	};
}
var ji = ie | oe;
function Mi(e, t, n, r) {
	new Ni(e, t, n, r);
}
var Ni = class {
		parent;
		is_pending = !1;
		transform_error;
		#e;
		#t = T ? D : null;
		#n;
		#r;
		#i;
		#a = null;
		#o = null;
		#s = null;
		#c = null;
		#l = 0;
		#u = 0;
		#d = !1;
		#f = new Set();
		#p = new Set();
		#m = null;
		#h = Ai(
			() => (
				(this.#m = On(this.#l)),
				() => {
					this.#m = null;
				}
			)
		);
		constructor(e, t, n, r) {
			((this.#e = e),
				(this.#n = t),
				(this.#r = (e) => {
					var t = K;
					((t.b = this), (t.f |= 128), n(e));
				}),
				(this.parent = K.b),
				(this.transform_error = r ?? this.parent?.transform_error ?? ((e) => e)),
				(this.#i = mr(() => {
					if (T) {
						let e = this.#t;
						k();
						let t = e.data === `[!`;
						if (e.data.startsWith(`[?`)) {
							let t = JSON.parse(e.data.slice(2));
							this.#_(t);
						} else t ? this.#y() : this.#g();
					} else this.#b();
				}, ji)),
				T && (this.#e = D));
		}
		#g() {
			try {
				this.#a = V(() => this.#r(this.#e));
			} catch (e) {
				this.error(e);
			}
		}
		#_(e) {
			let t = this.#n.failed,
				{ reset: n, invoke_onerror: r } = this.#v(e);
			(j(r),
				t &&
					(this.#s = V(() => {
						t(
							this.#e,
							() => e,
							() => n
						);
					})));
		}
		#v(e) {
			var t = !1,
				n = !1;
			let r = () => {
				if (t) {
					Pe();
					return;
				}
				((t = !0),
					n && ot(),
					this.#s !== null &&
						xr(this.#s, () => {
							this.#s = null;
						}),
					this.#S(() => {
						this.#b();
					}));
			};
			return {
				reset: r,
				invoke_onerror: () => {
					try {
						((n = !0), this.#n.onerror?.(e, r), (n = !1));
					} catch (e) {
						z(e, this.#i && this.#i.parent);
					}
				}
			};
		}
		#y() {
			let e = this.#n.pending;
			e &&
				((this.is_pending = !0),
				(this.#o = V(() => e(this.#e))),
				j(() => {
					var e = (this.#c = document.createDocumentFragment()),
						t = I(),
						n = !1;
					if (
						(e.append(t),
						(this.#a = this.#S(() => {
							try {
								return V(() => this.#r(t));
							} catch (e) {
								try {
									(this.error(e), (n = !0));
								} catch (e) {
									z(e, this.#i.parent);
								}
								return null;
							}
						})),
						this.#a === null)
					) {
						((this.#c = null), n && this.#x(N));
						return;
					}
					this.#u === 0 &&
						(this.#e.before(e),
						(this.#c = null),
						xr(this.#o, () => {
							this.#o = null;
						}),
						this.#x(N));
				}));
		}
		#b() {
			try {
				if (
					((this.is_pending = this.has_pending_snippet()),
					(this.#u = 0),
					(this.#l = 0),
					(this.#a = V(() => {
						this.#r(this.#e);
					})),
					this.#u > 0)
				) {
					var e = (this.#c = document.createDocumentFragment());
					Tr(this.#a, e);
					let t = this.#n.pending;
					this.#o = V(() => t(this.#e));
				} else this.#x(N);
			} catch (e) {
				this.error(e);
			}
		}
		#x(e) {
			((this.is_pending = !1), e.transfer_effects(this.#f, this.#p));
		}
		defer_effect(e) {
			Pt(e, this.#f, this.#p);
		}
		is_rendered() {
			return !this.is_pending && (!this.parent || this.parent.is_rendered());
		}
		has_pending_snippet() {
			return !!this.#n.pending;
		}
		#S(e) {
			var t = K,
				n = U,
				r = A;
			(q(this.#i), G(this.#i), vt(this.#i.ctx));
			try {
				return (mn.ensure(), e());
			} finally {
				(q(t), G(n), vt(r));
			}
		}
		#C(e, t) {
			if (!this.has_pending_snippet()) {
				this.parent && this.parent.#C(e, t);
				return;
			}
			((this.#u += e),
				this.#u === 0 &&
					(this.#x(t),
					this.#o &&
						xr(this.#o, () => {
							this.#o = null;
						}),
					(this.#c &&= (this.#e.before(this.#c), null))));
		}
		update_pending_count(e, t) {
			(this.#C(e, t),
				(this.#l += e),
				!(!this.#m || this.#d) &&
					((this.#d = !0),
					j(() => {
						((this.#d = !1), this.#m && jn(this.#m, this.#l));
					})));
		}
		get_effect_pending() {
			return (this.#h(), Q(this.#m));
		}
		error(e) {
			if (!this.#n.onerror && !this.#n.failed) throw e;
			N?.is_fork
				? (this.#a && N.skip_effect(this.#a),
					this.#o && N.skip_effect(this.#o),
					this.#s && N.skip_effect(this.#s),
					N.oncommit(() => {
						this.#w(e);
					}))
				: this.#w(e);
		}
		#w(e) {
			((this.#a &&= (H(this.#a), null)),
				(this.#o &&= (H(this.#o), null)),
				(this.#s &&= (H(this.#s), null)),
				T && (O(this.#t), Ie(), O(Le())));
			let t = this.#n.failed,
				n = (e) => {
					let { reset: n, invoke_onerror: r } = this.#v(e);
					(r(),
						t &&
							(this.#s = this.#S(() => {
								try {
									return V(() => {
										var r = K;
										((r.b = this),
											(r.f |= 128),
											t(
												this.#e,
												() => e,
												() => n
											));
									});
								} catch (e) {
									return (z(e, this.#i.parent), null);
								}
							})));
				};
			j(() => {
				var t;
				try {
					t = this.transform_error(e);
				} catch (e) {
					z(e, this.#i && this.#i.parent);
					return;
				}
				typeof t == `object` && t && typeof t.then == `function`
					? t.then(n, (e) => z(e, this.#i && this.#i.parent))
					: n(t);
			});
		}
	},
	Pi = !0;
function Fi(e) {
	Pi = e;
}
function Ii(e, t) {
	var n = t == null ? `` : typeof t == `object` ? `${t}` : t;
	n !== (e[xe] ??= e.nodeValue) && ((e[xe] = n), (e.nodeValue = `${n}`));
}
function Li(e, t) {
	return Bi(e, t);
}
function Ri(e, t) {
	(Wn(), (t.intro = t.intro ?? !1));
	let n = t.target,
		r = T,
		i = D;
	try {
		for (var a = L(n); a && (a.nodeType !== 8 || a.data !== `[`); ) a = R(a);
		if (!a) throw Te;
		(E(!0), O(a));
		let r = Bi(e, { ...t, anchor: a });
		return (E(!1), r);
	} catch (r) {
		if (
			r instanceof Error &&
			r.message
				.split(
					`
`
				)
				.some((e) => e.startsWith(`https://svelte.dev/e/`))
		)
			throw r;
		return (
			r !== Te && console.warn(`Failed to hydrate: `, r),
			t.recover === !1 && et(),
			Wn(),
			Yn(n),
			E(!1),
			Li(e, t)
		);
	} finally {
		(E(r), O(i));
	}
}
var zi = new Map();
function Bi(
	e,
	{
		target: t,
		anchor: n,
		props: r = {},
		events: i,
		context: o,
		intro: s = !0,
		transformError: c
	}
) {
	Wn();
	var l = void 0,
		u = cr(() => {
			var u = n ?? t.appendChild(I());
			Mi(
				u,
				{ pending: () => {} },
				(t) => {
					wt({});
					var n = A;
					if (
						(o && (n.c = o),
						i && (r.$$events = i),
						T && $(t, null),
						(Pi = s),
						(l = e(t, r) || Et()),
						(Pi = !0),
						T && ((K.nodes.end = D), D === null || D.nodeType !== 8 || D.data !== `]`))
					)
						throw (Me(), Te);
					Tt();
				},
				c
			);
			var d = new Set(),
				f = (e) => {
					for (var n = 0; n < e.length; n++) {
						var r = e[n];
						if (!d.has(r)) {
							d.add(r);
							var i = ai(r);
							for (let e of [t, document]) {
								var a = zi.get(e);
								a === void 0 && ((a = new Map()), zi.set(e, a));
								var o = a.get(r);
								o === void 0
									? (e.addEventListener(r, yi, { passive: i }), a.set(r, 1))
									: a.set(r, o + 1);
							}
						}
					}
				};
			return (
				f(a(li)),
				ui.add(f),
				() => {
					for (var e of d)
						for (let n of [t, document]) {
							var r = zi.get(n),
								i = r.get(e);
							--i == 0
								? (n.removeEventListener(e, yi),
									r.delete(e),
									r.size === 0 && zi.delete(n))
								: r.set(e, i);
						}
					(ui.delete(f), u !== n && u.parentNode?.removeChild(u));
				}
			);
		});
	return (Vi.set(l, u), l);
}
var Vi = new WeakMap();
function Hi(e, t) {
	let n = Vi.get(e);
	return n ? (Vi.delete(e), n(t)) : Promise.resolve();
}
var Ui = class {
	anchor;
	#e = new Map();
	#t = new Map();
	#n = new Map();
	#r = new Set();
	#i = !0;
	constructor(e, t = !0) {
		((this.anchor = e), (this.#i = t));
	}
	#a = (e) => {
		if (this.#e.has(e)) {
			var t = this.#e.get(e),
				n = this.#t.get(t);
			if (n) (Cr(n), this.#r.delete(t));
			else {
				var r = this.#n.get(t);
				r &&
					(Cr(r.effect),
					this.#t.set(t, r.effect),
					this.#n.delete(t),
					r.fragment.lastChild.remove(),
					this.anchor.before(r.fragment),
					(n = r.effect));
			}
			for (let [t, n] of this.#e) {
				if ((this.#e.delete(t), t === e)) break;
				let r = this.#n.get(n);
				r && (H(r.effect), this.#n.delete(n));
			}
			for (let [e, r] of this.#t) {
				if (e === t || this.#r.has(e)) continue;
				let i = () => {
					if (Array.from(this.#e.values()).includes(e)) {
						var t = document.createDocumentFragment();
						(Tr(r, t), t.append(I()), this.#n.set(e, { effect: r, fragment: t }));
					} else H(r);
					(this.#r.delete(e), this.#t.delete(e));
				};
				this.#i || !n ? (this.#r.add(e), xr(r, i, !1)) : i();
			}
		}
	};
	#o = (e) => {
		this.#e.delete(e);
		let t = Array.from(this.#e.values());
		for (let [e, n] of this.#n) t.includes(e) || (H(n.effect), this.#n.delete(e));
	};
	ensure(e, t) {
		var n = N,
			r = Xn();
		if (t && !this.#t.has(e) && !this.#n.has(e)) {
			if (r) {
				var i = document.createDocumentFragment(),
					a = I();
				(i.append(a), this.#n.set(e, { effect: V(() => t(a)), fragment: i }));
			} else
				this.#t.set(
					e,
					V(() => t(this.anchor))
				);
		}
		if ((this.#e.set(n, e), r)) {
			for (let [t, r] of this.#t) t === e ? n.unskip_effect(r) : n.skip_effect(r);
			for (let [t, r] of this.#n)
				t === e ? n.unskip_effect(r.effect) : n.skip_effect(r.effect);
			(n.oncommit(this.#a), n.ondiscard(this.#o));
		} else (T && (this.anchor = D), this.#a(n));
	}
};
function Wi(e, t, n = !1) {
	var r;
	T && ((r = D), k());
	var i = new Ui(e),
		a = n ? ie : 0;
	function o(e, t) {
		if (T) {
			var n = Re(r);
			if (e !== parseInt(n.substring(1))) {
				var a = Le();
				(O(a), (i.anchor = a), E(!1), i.ensure(e, t), E(!0));
				return;
			}
		}
		i.ensure(e, t);
	}
	mr(() => {
		var e = !1;
		(t((t, n = 0) => {
			((e = !0), o(n, t));
		}),
			e || o(-1, null));
	}, a);
}
var Gi = Symbol(`NaN`);
function Ki(e, t, n) {
	T && k();
	var r = new Ui(e),
		i = !Dt();
	mr(() => {
		var e = t();
		(e !== e && (e = Gi), i && typeof e == `object` && e && (e = {}), r.ensure(e, n));
	});
}
function qi(e, t) {
	return t;
}
function Ji(e, t, n) {
	for (var r = [], i = t.length, o, s = t.length, c = 0; c < i; c++) {
		let n = t[c];
		xr(
			n,
			() => {
				if (o) {
					if ((o.pending.delete(n), o.done.add(n), o.pending.size === 0)) {
						var t = e.outrogroups;
						(Yi(e, a(o.done)), t.delete(o), t.size === 0 && (e.outrogroups = null));
					}
				} else --s;
			},
			!1
		);
	}
	if (s === 0) {
		var l = r.length === 0 && n !== null && e.pending.size === 0;
		if (l) {
			var u = n,
				d = u.parentNode;
			(Yn(d), d.append(u), e.items.clear());
		}
		Yi(e, t, !l);
	} else
		((o = { pending: new Set(t), done: new Set() }),
			(e.outrogroups ??= new Set()).add(o));
}
function Yi(e, t, n = !0) {
	var r;
	if (e.pending.size > 0) {
		r = new Set();
		for (let t of e.pending.values()) for (let n of t) r.add(e.items.get(n).e);
	}
	for (var i = 0; i < t.length; i++) {
		var a = t[i];
		r?.has(a) ? ((a.f |= ce), Tr(a, document.createDocumentFragment())) : H(t[i], n);
	}
}
var Xi;
function Zi(e, t, r, i, o, s = null) {
	var c = e,
		l = new Map();
	if (t & 4) {
		var u = e;
		c = T ? O(L(u)) : u.appendChild(I());
	}
	T && k();
	var d = null,
		f = Qt(() => {
			var e = r();
			return n(e) ? e : e == null ? [] : a(e);
		}),
		p,
		m = new Map(),
		h = !0;
	function g(e) {
		v.effect.f & 16384 ||
			(v.pending.delete(e),
			(v.fallback = d),
			$i(v, p, c, t, i),
			d !== null &&
				(p.length === 0
					? d.f & 33554432
						? ((d.f ^= ce), ta(d, null, c))
						: Cr(d)
					: xr(d, () => {
							d = null;
						})));
	}
	function _(e) {
		v.pending.delete(e);
	}
	var v = {
		effect: mr(() => {
			p = Q(f);
			var e = p.length;
			let n = !1;
			T && (Re(c) === `[!`) != (e === 0) && ((c = Le()), O(c), E(!1), (n = !0));
			for (var a = new Set(), u = N, v = Xn(), y = 0; y < e; y += 1) {
				T && D.nodeType === 8 && D.data === `]` && ((c = D), (n = !0), E(!1));
				var b = p[y],
					x = i(b, y),
					S = h ? null : l.get(x);
				(S
					? (S.v && jn(S.v, b), S.i && jn(S.i, y), v && u.unskip_effect(S.e))
					: ((S = ea(l, h ? c : (Xi ??= I()), b, x, y, o, t, r)),
						h || (S.e.f |= ce),
						l.set(x, S)),
					a.add(x));
			}
			if (
				(e === 0 &&
					s &&
					!d &&
					(h ? (d = V(() => s(c))) : ((d = V(() => s((Xi ??= I())))), (d.f |= ce))),
				e > a.size && Ke(``, ``, ``),
				T && e > 0 && O(Le()),
				!h)
			) {
				if ((m.set(u, a), v)) {
					for (let [e, t] of l) a.has(e) || u.skip_effect(t.e);
					(u.oncommit(g), u.ondiscard(_));
				} else g(u);
			}
			(n && E(!0), Q(f));
		}),
		flags: t,
		items: l,
		pending: m,
		outrogroups: null,
		fallback: d
	};
	((h = !1), T && (c = D));
}
function Qi(e) {
	for (; e !== null && !(e.f & 32); ) e = e.next;
	return e;
}
function $i(e, t, n, r, i) {
	var o = !!(r & 8),
		s = t.length,
		c = e.items,
		l = Qi(e.effect.first),
		u,
		d = null,
		f,
		p = [],
		m = [],
		h,
		g,
		_,
		v;
	if (o)
		for (v = 0; v < s; v += 1)
			((h = t[v]),
				(g = i(h, v)),
				(_ = c.get(g).e),
				_.f & 33554432 || (_.nodes?.a?.measure(), (f ??= new Set()).add(_)));
	for (v = 0; v < s; v += 1) {
		if (((h = t[v]), (g = i(h, v)), (_ = c.get(g).e), e.outrogroups !== null))
			for (let t of e.outrogroups) (t.pending.delete(_), t.done.delete(_));
		if (
			(_.f & 8192 && (Cr(_), o && (_.nodes?.a?.unfix(), (f ??= new Set()).delete(_))),
			_.f & 33554432)
		) {
			if (((_.f ^= ce), _ === l)) ta(_, null, n);
			else {
				var y = d ? d.next : l;
				(_ === e.effect.last && (e.effect.last = _.prev),
					_.prev && (_.prev.next = _.next),
					_.next && (_.next.prev = _.prev),
					na(e, d, _),
					na(e, _, y),
					ta(_, y, n),
					(d = _),
					(p = []),
					(m = []),
					(l = Qi(d.next)));
				continue;
			}
		}
		if (_ !== l) {
			if (u !== void 0 && u.has(_)) {
				if (p.length < m.length) {
					var b = m[0],
						x;
					d = b.prev;
					var S = p[0],
						C = p[p.length - 1];
					for (x = 0; x < p.length; x += 1) ta(p[x], b, n);
					for (x = 0; x < m.length; x += 1) u.delete(m[x]);
					(na(e, S.prev, C.next),
						na(e, d, S),
						na(e, C, b),
						(l = b),
						(d = C),
						--v,
						(p = []),
						(m = []));
				} else
					(u.delete(_),
						ta(_, l, n),
						na(e, _.prev, _.next),
						na(e, _, d === null ? e.effect.first : d.next),
						na(e, d, _),
						(d = _));
				continue;
			}
			for (p = [], m = []; l !== null && l !== _; )
				((u ??= new Set()).add(l), m.push(l), (l = Qi(l.next)));
			if (l === null) continue;
		}
		(_.f & 33554432 || p.push(_), (d = _), (l = Qi(_.next)));
	}
	if (e.outrogroups !== null) {
		for (let t of e.outrogroups)
			t.pending.size === 0 && (Yi(e, a(t.done)), e.outrogroups?.delete(t));
		e.outrogroups.size === 0 && (e.outrogroups = null);
	}
	if (l !== null || u !== void 0) {
		var ee = [];
		if (u !== void 0) for (_ of u) _.f & 8192 || ee.push(_);
		for (; l !== null; )
			(!(l.f & 8192) && l !== e.fallback && ee.push(l), (l = Qi(l.next)));
		var te = ee.length;
		if (te > 0) {
			var ne = r & 4 && s === 0 ? n : null;
			if (o) {
				for (v = 0; v < te; v += 1) ee[v].nodes?.a?.measure();
				for (v = 0; v < te; v += 1) ee[v].nodes?.a?.fix();
			}
			Ji(e, ee, ne);
		}
	}
	o &&
		j(() => {
			if (f !== void 0) for (_ of f) _.nodes?.a?.apply();
		});
}
function ea(e, t, n, r, i, a, o, s) {
	var c = o & 1 ? (o & 16 ? On(n) : An(n, !1, !1)) : null,
		l = o & 2 ? On(i) : null;
	return {
		v: c,
		i: l,
		e: V(
			() => (
				a(t, c ?? n, l ?? i, s),
				() => {
					e.delete(r);
				}
			)
		)
	};
}
function ta(e, t, n) {
	if (e.nodes)
		for (
			var r = e.nodes.start,
				i = e.nodes.end,
				a = t && !(t.f & 33554432) ? t.nodes.start : n;
			r !== null;
		) {
			var o = R(r);
			if ((a.before(r), r === i)) return;
			r = o;
		}
}
function na(e, t, n) {
	(t === null ? (e.effect.first = n) : (t.next = n),
		n === null ? (e.effect.last = t) : (n.prev = t));
}
function ra(e, t, n = !1, r = !1, i = !1, a = !1) {
	var o = e,
		s = ``;
	if (n) {
		var c = e;
		T && (o = O(L(c)));
	}
	fr(() => {
		var e = K;
		if (s === (s = t() ?? ``)) {
			T && k();
			return;
		}
		if (n && !T) {
			((e.nodes = null), (c.innerHTML = s), s !== `` && $(L(c), c.lastChild));
			return;
		}
		if (
			(e.nodes !== null && (yr(e.nodes.start, e.nodes.end), (e.nodes = null)), s !== ``)
		) {
			if (T) {
				for (
					var a = D.data, l = k(), u = l;
					l !== null && (l.nodeType !== 8 || l.data !== ``);
				)
					((u = l), (l = R(l)));
				if (l === null) throw (Me(), Te);
				($(D, u), (o = O(l)));
				return;
			}
			var d = Zn(r ? `svg` : i ? `math` : `template`, r ? De : i ? Oe : void 0);
			d.innerHTML = s;
			var f = r || i ? d : d.content;
			if (($(L(f), f.lastChild), r || i)) for (; L(f); ) o.before(L(f));
			else o.before(f);
		}
	});
}
function ia(e, t, ...n) {
	var r = new Ui(e);
	mr(() => {
		let e = t() ?? null;
		r.ensure(e, e && ((t) => e(t, ...n)));
	}, ie);
}
function aa(e) {
	return (t, ...n) => {
		var r = e(...n),
			i;
		T ? ((i = D), k()) : ((i = L(Si(r.render().trim()))), t.before(i));
		let a = r.setup?.(i);
		($(i, i), typeof a == `function` && rr(a));
	};
}
function oa(e, t, n) {
	var r;
	T && ((r = D), k());
	var i = new Ui(e);
	mr(() => {
		var e = t() ?? null;
		if (T && (Re(r) === `[`) != (e !== null)) {
			var a = Le();
			(O(a), (i.anchor = a), E(!1), i.ensure(e, e && ((t) => n(t, e))), E(!0));
			return;
		}
		i.ensure(e, e && ((t) => n(t, e)));
	}, ie);
}
var sa = () => performance.now(),
	ca = { tick: (e) => requestAnimationFrame(e), now: () => sa(), tasks: new Set() };
function la() {
	let e = ca.now();
	(ca.tasks.forEach((t) => {
		t.c(e) || (ca.tasks.delete(t), t.f());
	}),
		ca.tasks.size !== 0 && ca.tick(la));
}
function ua(e) {
	let t;
	return (
		ca.tasks.size === 0 && ca.tick(la),
		{
			promise: new Promise((n) => {
				ca.tasks.add((t = { c: e, f: n }));
			}),
			abort() {
				ca.tasks.delete(t);
			}
		}
	);
}
function da(e, t) {
	Ht(() => {
		e.dispatchEvent(new CustomEvent(t));
	});
}
function fa(e) {
	if (e === `float`) return `cssFloat`;
	if (e === `offset`) return `cssOffset`;
	if (e.startsWith(`--`)) return e;
	let t = e.split(`-`);
	return t.length === 1
		? t[0]
		: t[0] +
				t
					.slice(1)
					.map((e) => e[0].toUpperCase() + e.slice(1))
					.join(``);
}
function pa(e) {
	let t = {},
		n = e.split(`;`);
	for (let e of n) {
		let [n, r] = e.split(`:`);
		if (!n || r === void 0) break;
		let i = fa(n.trim());
		t[i] = r.trim();
	}
	return t;
}
var ma = (e) => e;
function ha(e, t, n, r) {
	var i = !!(e & 1),
		a = !!(e & 2),
		o = i && a,
		s = !!(e & 4),
		c = o ? `both` : i ? `in` : `out`,
		l,
		u = t.inert,
		d = t.style.overflow,
		f,
		p;
	function m() {
		return Ht(() => (l ??= n()(t, r?.() ?? {}, { direction: c })));
	}
	var h = {
			is_global: s,
			in() {
				if (((t.inert = u), !i)) {
					(p?.abort(), p?.reset?.());
					return;
				}
				(a || f?.abort(),
					(f = ga(
						t,
						m(),
						p,
						1,
						() => {
							da(t, `introstart`);
						},
						() => {
							(da(t, `introend`), f?.abort(), (f = l = void 0), (t.style.overflow = d));
						}
					)));
			},
			out(e) {
				if (!a) {
					(e?.(), (l = void 0));
					return;
				}
				((t.inert = !0),
					(p = ga(
						t,
						m(),
						f,
						0,
						() => {
							da(t, `outrostart`);
						},
						() => {
							(da(t, `outroend`), e?.());
						}
					)));
			},
			stop: () => {
				(f?.abort(), p?.abort());
			}
		},
		g = K;
	if (((g.nodes.t ??= []).push(h), i && Pi)) {
		var _ = s;
		if (!_) {
			for (var v = g.parent; v && v.f & 65536; ) for (; (v = v.parent) && !(v.f & 16); );
			_ = !v || !!(v.f & 32768);
		}
		_ &&
			lr(() => {
				Jr(() => h.in());
			});
	}
}
function ga(e, t, n, r, i, a) {
	var o = r === 1,
		s = !1;
	if (p(t)) {
		var c;
		return (
			j(() => {
				s || (c = ga(e, t({ direction: o ? `in` : `out` }), n, r, i, a));
			}),
			{
				abort: () => {
					((s = !0), c?.abort());
				},
				deactivate: () => c.deactivate(),
				reset: () => c.reset(),
				t: () => c.t()
			}
		);
	}
	if ((n?.deactivate(), !t?.duration && !t?.delay))
		return (i(), a(), { abort: m, deactivate: m, reset: m, t: () => r });
	let { delay: l = 0, css: u, tick: d, easing: f = ma } = t;
	var h,
		g = () => 1 - r;
	return (
		j(() => {
			if (!s) {
				var c = [];
				if (o && n === void 0 && (d && d(0, 1), u)) {
					var p = pa(u(0, 1));
					c.push(p, p);
				}
				((h = e.animate(c, { duration: l, fill: `forwards` })),
					(h.onfinish = () => {
						(h.cancel(), i());
						var o = n?.t() ?? 1 - r;
						n?.abort();
						var s = r - o,
							c = t.duration * Math.abs(s),
							l = [];
						if (c > 0) {
							var p = !1;
							if (u)
								for (var m = Math.ceil(c / (1e3 / 60)), _ = 0; _ <= m; _ += 1) {
									var v = o + s * f(_ / m),
										y = pa(u(v, 1 - v));
									(l.push(y), (p ||= y.overflow === `hidden`));
								}
							(p && (e.style.overflow = `hidden`),
								(g = () => {
									var e = h.currentTime;
									return o + s * f(e / c);
								}),
								d &&
									ua(() => {
										if (h.playState !== `running`) return !1;
										var e = g();
										return (d(e, 1 - e), !0);
									}));
						}
						((h = e.animate(l, { duration: c, fill: `forwards` })),
							(h.onfinish = () => {
								((g = () => r), d?.(r, 1 - r), a());
							}));
					}));
			}
		}),
		{
			abort: () => {
				((s = !0), h && (h.cancel(), (h.effect = null), (h.onfinish = m)));
			},
			deactivate: () => {
				a = m;
			},
			reset: () => {
				r === 0 && d?.(1, 0);
			},
			t: () => g()
		}
	);
}
function _a(e, t, n, r, i, a) {
	let o = T;
	T && k();
	var s = null;
	T && D.nodeType === 1 && ((s = D), k());
	var c = T ? D : e,
		l = new Ui(c, !1);
	(mr(() => {
		let e = t() || null;
		var a = i ? i() : n || e === `svg` ? De : void 0;
		if (e === null) {
			(l.ensure(null, null), Fi(!0));
			return;
		}
		return (
			l.ensure(e, (t) => {
				if (e) {
					if (((s = T ? s : Zn(e, a)), $(s, s), r)) {
						var n = null;
						T && si(e) && s.append((n = document.createComment(``)));
						var i = T ? L(s) : s.appendChild(I());
						(T && (i === null ? E(!1) : O(i)), r(s, i), n?.remove());
					}
					((K.nodes.end = s), t.before(s));
				}
				T && O(t);
			}),
			Fi(!0),
			() => {
				e && Fi(!1);
			}
		);
	}, ie),
		rr(() => {
			Fi(!0);
		}),
		o && (E(!0), O(c)));
}
function va(e, t) {
	let n = null,
		r = T;
	var i;
	if (T) {
		n = D;
		for (var a = L(document.head); a !== null && (a.nodeType !== 8 || a.data !== e); )
			a = R(a);
		if (a === null) E(!1);
		else {
			var o = R(a);
			(a.remove(), O(o));
		}
	}
	T || (i = document.head.appendChild(I()));
	try {
		mr(() => {
			var e = V(() => t(i));
			((e.f |= ae),
				T ||
					(e.nodes === null
						? (e.nodes = { start: i, end: i, a: null, t: null })
						: (e.nodes.end = i)));
		});
	} finally {
		r && (E(!0), O(n));
	}
}
function ya(e, t) {
	var n = void 0,
		r;
	hr(() => {
		n !== (n = t()) &&
			((r &&= (H(r), null)),
			n &&
				(r = V(() => {
					lr(() => n(e));
				})));
	});
}
function ba(e) {
	var t,
		n,
		r = ``;
	if (typeof e == `string` || typeof e == `number`) r += e;
	else if (typeof e == `object`) {
		if (Array.isArray(e)) {
			var i = e.length;
			for (t = 0; t < i; t++) e[t] && (n = ba(e[t])) && (r && (r += ` `), (r += n));
		} else for (n in e) e[n] && (r && (r += ` `), (r += n));
	}
	return r;
}
function xa() {
	for (var e, t, n = 0, r = ``, i = arguments.length; n < i; n++)
		(e = arguments[n]) && (t = ba(e)) && (r && (r += ` `), (r += t));
	return r;
}
function Sa(e) {
	return typeof e == `object` ? xa(e) : (e ?? ``);
}
var Ca = [
	...` 	
\r\f\xA0\v﻿`
];
function wa(e, t, n) {
	var r = e == null ? `` : `` + e;
	if ((t && (r = r ? r + ` ` + t : t), n)) {
		for (var i of Object.keys(n))
			if (n[i]) r = r ? r + ` ` + i : i;
			else if (r.length)
				for (var a = i.length, o = 0; (o = r.indexOf(i, o)) >= 0; ) {
					var s = o + a;
					(o === 0 || Ca.includes(r[o - 1])) && (s === r.length || Ca.includes(r[s]))
						? (r = (o === 0 ? `` : r.substring(0, o)) + r.substring(s + 1))
						: (o = s);
				}
	}
	return r === `` ? null : r;
}
function Ta(e, t = !1) {
	var n = t ? ` !important;` : `;`,
		r = ``;
	for (var i of Object.keys(e)) {
		var a = e[i];
		a != null && a !== `` && (r += ` ` + i + `: ` + a + n);
	}
	return r;
}
function Ea(e) {
	return e[0] !== `-` || e[1] !== `-` ? e.toLowerCase() : e;
}
function Da(e, t) {
	if (t) {
		var n = ``,
			r,
			i;
		if ((Array.isArray(t) ? ((r = t[0]), (i = t[1])) : (r = t), e)) {
			e = String(e)
				.replaceAll(/\/\*.*?\*\//g, ``)
				.trim();
			var a = !1,
				o = 0,
				s = !1,
				c = [];
			(r && c.push(...Object.keys(r).map(Ea)), i && c.push(...Object.keys(i).map(Ea)));
			var l = 0,
				u = -1;
			let t = e.length;
			for (var d = 0; d < t; d++) {
				var f = e[d];
				if (
					(s
						? f === `/` && e[d - 1] === `*` && (s = !1)
						: a
							? a === f && (a = !1)
							: f === `/` && e[d + 1] === `*`
								? (s = !0)
								: f === `"` || f === `'`
									? (a = f)
									: f === `(`
										? o++
										: f === `)` && o--,
					!s && a === !1 && o === 0)
				) {
					if (f === `:` && u === -1) u = d;
					else if (f === `;` || d === t - 1) {
						if (u !== -1) {
							var p = Ea(e.substring(l, u).trim());
							if (!c.includes(p)) {
								f !== `;` && d++;
								var m = e.substring(l, d).trim();
								n += ` ` + m + `;`;
							}
						}
						((l = d + 1), (u = -1));
					}
				}
			}
		}
		return (
			r && (n += Ta(r)),
			i && (n += Ta(i, !0)),
			(n = n.trim()),
			n === `` ? null : n
		);
	}
	return e == null ? null : String(e);
}
function Oa(e, t, n, r, i, a) {
	var o = e[ye];
	if (T || o !== n || o === void 0) {
		var s = wa(n, r, a);
		((!T || s !== e.getAttribute(`class`)) &&
			(s == null
				? e.removeAttribute(`class`)
				: t
					? (e.className = s)
					: e.setAttribute(`class`, s)),
			(e[ye] = n));
	} else if (a && i !== a)
		for (var c in a) {
			var l = !!a[c];
			(i == null || l !== !!i[c]) && e.classList.toggle(c, l);
		}
	return a;
}
function ka(e, t = {}, n, r) {
	for (var i in n) {
		var a = n[i];
		t[i] !== a &&
			(n[i] == null ? e.style.removeProperty(i) : e.style.setProperty(i, a, r));
	}
}
function Aa(e, t, n, r) {
	var i = e[be];
	if (T || i !== t) {
		var a = Da(t, r);
		((!T || a !== e.getAttribute(`style`)) &&
			(a == null ? e.removeAttribute(`style`) : (e.style.cssText = a)),
			(e[be] = t));
	} else
		r &&
			(Array.isArray(r)
				? (ka(e, n?.[0], r[0]), ka(e, n?.[1], r[1], `important`))
				: ka(e, n, r));
	return r;
}
function ja(e, t) {
	t
		? e.hasAttribute(`selected`) || e.setAttribute(`selected`, ``)
		: e.removeAttribute(`selected`);
}
function Ma(e, t) {
	var n = !(`__defaultValue` in e);
	(n || e.__defaultValue !== t) && ((e.__defaultValue = t), Na(e, !n || `__value` in e));
}
function Na(e, t) {
	var r = e.__defaultValue,
		i = e.multiple,
		a = i ? (r ?? []) : null;
	if (!i || n(a)) {
		var o = e.selectedIndex,
			s = t && i ? new Set(e.selectedOptions) : null;
		for (var c of e.options) {
			var l = Ia(c);
			ja(c, i ? a.includes(l) : Rn(l, r));
		}
		if (t) {
			if (s !== null)
				for (c of e.options) {
					var u = s.has(c);
					c.selected !== u && (c.selected = u);
				}
			else e.selectedIndex !== o && (e.selectedIndex = o);
		}
	}
}
function Pa(e, t, r = !1) {
	if (e.multiple) {
		if (t == null) return;
		if (!n(t)) return Ne();
		for (var i of e.options) i.selected = t.includes(Ia(i));
		return;
	}
	for (i of e.options)
		if (Rn(Ia(i), t)) {
			i.selected = !0;
			return;
		}
	(!r || t !== void 0) && (e.selectedIndex = -1);
}
function Fa(e) {
	var t = new MutationObserver((t) => {
		t.every(La) ||
			(`__defaultValue` in e && Na(e, !1), `__value` in e && Pa(e, e.__value));
	});
	(t.observe(e, {
		childList: !0,
		subtree: !0,
		attributes: !0,
		attributeFilter: [`value`]
	}),
		rr(() => {
			t.disconnect();
		}));
}
function Ia(e) {
	return `__value` in e ? e.__value : e.value;
}
function La(e) {
	if (e.target.closest(`selectedcontent`) !== null) return !0;
	if (e.type === `childList`) {
		var t = [...e.addedNodes, ...e.removedNodes];
		return t.length > 0 && t.every((e) => e.nodeName === `SELECTEDCONTENT`);
	}
	return !1;
}
var Ra = Symbol(`class`),
	za = Symbol(`style`),
	Ba = Symbol(`is custom element`),
	Va = Symbol(`is html`),
	Ha = we ? `link` : `LINK`,
	Ua = we ? `input` : `INPUT`,
	Wa = we ? `option` : `OPTION`,
	Ga = we ? `select` : `SELECT`;
function Ka(e) {
	if (T) {
		var t = !1,
			n = () => {
				if (!t) {
					if (((t = !0), e.hasAttribute(`value`))) {
						var n = e.value;
						(qa(e, `value`, null), (e.value = n));
					}
					if (e.hasAttribute(`checked`)) {
						var r = e.checked;
						(qa(e, `checked`, null), (e.checked = r));
					}
				}
			};
		((e[Se] = n), j(n), Vt());
	}
}
function qa(e, t, n, r) {
	var i = Xa(e);
	(T &&
		((i[t] = e.getAttribute(t)),
		t === `src` || t === `srcset` || (t === `href` && e.nodeName === Ha))) ||
		(i[t] !== (i[t] = n) &&
			(t === `loading` && (e[ge] = n),
			n == null
				? e.removeAttribute(t)
				: typeof n != `string` && Qa(e).has(t)
					? (e[t] = n)
					: e.setAttribute(t, n)));
}
function Ja(e, t, n, r, i = !1, a = !1) {
	T && i && e.nodeName === Ua && (`defaultValue` in n || `defaultChecked` in n || Ka(e));
	var o = Xa(e),
		s = o[Ba],
		c = !o[Va];
	let l = T && s;
	l && E(!1);
	var u = t || {},
		d = e.nodeName === Wa,
		f = e.nodeName === Ga;
	for (var p in t) !(p in n) && p[0] + p[1] !== `$$` && (n[p] = null);
	(n.class ? (n.class = Sa(n.class)) : (r || n[Ra]) && (n.class = null),
		n[za] && (n.style ??= null));
	var m = Qa(e);
	if (e.nodeName === Ua && `type` in n && (`value` in n || `__value` in n)) {
		var h = n.type;
		(h !== u.type || (h === void 0 && e.hasAttribute(`type`))) &&
			((u.type = h), qa(e, `type`, h, a));
	}
	for (let i in n) {
		let l = n[i];
		if (d && i === `value` && l == null) {
			((e.value = e.__value = ``), (u[i] = l));
			continue;
		}
		if (i === `class`) {
			(Oa(e, e.namespaceURI === `http://www.w3.org/1999/xhtml`, l, r, t?.[Ra], n[Ra]),
				(u[i] = l),
				(u[Ra] = n[Ra]));
			continue;
		}
		if (i === `style`) {
			(Aa(e, l, t?.[za], n[za]), (u[i] = l), (u[za] = n[za]));
			continue;
		}
		var g = u[i];
		if (l !== g || (l === void 0 && e.hasAttribute(i))) {
			u[i] = l;
			var _ = i[0] + i[1];
			if (_ !== `$$`) {
				if (_ === `on`) {
					let t = {},
						n = `$$` + i,
						r = i.slice(2);
					var v = ei(r);
					if ((Qr(r) && ((r = r.slice(0, -7)), (t.capture = !0)), !v && g)) {
						if (l != null) continue;
						(e.removeEventListener(r, u[n], t), (u[n] = null));
					}
					if (v) (hi(r, e, l), gi([r]));
					else if (l != null) {
						function a(e) {
							u[i].call(this, e);
						}
						u[n] = fi(r, e, a, t);
					}
				} else if (i === `style`) qa(e, i, l);
				else if (i === `autofocus`) zt(e, !!l);
				else if (!s && (i === `__value` || (i === `value` && l != null)))
					e.value = e.__value = l;
				else if (i === `selected` && d) ja(e, l);
				else {
					var y = i;
					c || (y = ri(y));
					var b = y === `defaultValue` || y === `defaultChecked`;
					if (f && y === `defaultValue`) continue;
					if (l == null && !s && !b) {
						if (((o[i] = null), y === `value` || y === `checked`)) {
							let n = e,
								r = t === void 0;
							if (y === `value`) {
								let e = n.defaultValue;
								(n.removeAttribute(y),
									(n.defaultValue = e),
									(n.value = n.__value = r ? e : null));
							} else {
								let e = n.defaultChecked;
								(n.removeAttribute(y), (n.defaultChecked = e), (n.checked = r ? e : !1));
							}
						} else e.removeAttribute(i);
					} else
						b || ((s || typeof l != `string`) && m.has(y))
							? ((e[y] = l), y in o && (o[y] = w))
							: typeof l != `function` && qa(e, y, l, a);
				}
			}
		}
	}
	return (l && E(!0), u);
}
function Ya(e, t, n = [], r = [], i = [], a, o = !1, s = !1) {
	Wt(i, n, r, (n) => {
		var r = void 0,
			i = {},
			c = e.nodeName === Ga,
			l = !1;
		if (
			(hr(() => {
				var u = t(...n.map(Q)),
					d = Ja(e, r, u, a, o, s);
				if (l && c) {
					var f = e;
					(`defaultValue` in u && Ma(f, u.defaultValue), `value` in u && Pa(f, u.value));
				}
				for (let e of Object.getOwnPropertySymbols(i)) u[e] || H(i[e]);
				for (let t of Object.getOwnPropertySymbols(u)) {
					var p = u[t];
					(t.description === `@attach` &&
						(!r || p !== r[t]) &&
						(i[t] && H(i[t]), (i[t] = V(() => ya(e, () => p)))),
						(d[t] = p));
				}
				r = d;
			}),
			c)
		) {
			var u = e;
			lr(() => {
				var e = r;
				(`defaultValue` in e && Ma(u, e.defaultValue), Pa(u, e.value, !0), Fa(u));
			});
		}
		l = !0;
	});
}
function Xa(e) {
	return (e[ve] ??= { [Ba]: e.nodeName.includes(`-`), [Va]: e.namespaceURI === Ee });
}
var Za = new Map();
function Qa(e) {
	var t = e.getAttribute(`is`) || e.nodeName,
		n = Za.get(t);
	if (n) return n;
	Za.set(t, (n = new Set()));
	for (var r, i = e, a = Element.prototype; a !== i; ) {
		for (var o in ((r = c(i)), r))
			r[o].set &&
				o !== `innerHTML` &&
				o !== `textContent` &&
				o !== `innerText` &&
				n.add(o);
		i = d(i);
	}
	return n;
}
function $a(e, t, n = t) {
	var r = new WeakSet();
	(Ut(e, `input`, async (i) => {
		var a = i ? e.defaultValue : e.value;
		if (
			((a = eo(e) ? to(a) : a), n(a), N !== null && r.add(N), await Wr(), a !== (a = t()))
		) {
			var o = e.selectionStart,
				s = e.selectionEnd,
				c = e.value.length;
			if (((e.value = a ?? ``), s !== null)) {
				var l = e.value.length;
				o === s && s === c && l > c
					? ((e.selectionStart = l), (e.selectionEnd = l))
					: ((e.selectionStart = o), (e.selectionEnd = Math.min(s, l)));
			}
		}
	}),
		((T && e.defaultValue !== e.value) || (Jr(t) == null && e.value)) &&
			(n(eo(e) ? to(e.value) : e.value), N !== null && r.add(N)),
		dr(() => {
			var n = t();
			if (e === document.activeElement) {
				var i = N;
				if (r.has(i)) return;
			}
			(eo(e) && n === to(e.value)) ||
				((e.type !== `date` || n || e.value) && n !== e.value && (e.value = n ?? ``));
		}));
}
function eo(e) {
	var t = e.type;
	return t === `number` || t === `range`;
}
function to(e) {
	return e === `` ? null : +e;
}
function no(e, t) {
	return e === t || e?.[pe] === t;
}
function ro(e = Et(), t, n, r) {
	var i = A.r,
		a = K;
	return (
		lr(() => {
			var o, s;
			return (
				dr(() => {
					((o = s),
						(s = r?.() || []),
						Jr(() => {
							no(n(...s), e) || (t(e, ...s), o && no(n(...o), e) && t(null, ...o));
						}));
				}),
				() => {
					let r = a;
					for (; r !== i && r.parent !== null && r.parent.f & 33554432; ) r = r.parent;
					let o = () => {
							s && no(n(...s), e) && t(null, ...s);
						},
						c = r.teardown;
					r.teardown = () => {
						(o(), c?.());
					};
				}
			);
		}),
		e
	);
}
function io(e = !1) {
	let t = A,
		n = t.l.u;
	if (!n) return;
	let r = () => Yr(t.s);
	if (e) {
		let e = 0,
			n = {},
			i = Jt(() => {
				let r = !1,
					i = t.s;
				for (let e in i) i[e] !== n[e] && ((n[e] = i[e]), (r = !0));
				return (r && e++, e);
			});
		r = () => Q(i);
	}
	(n.b.length &&
		or(() => {
			(ao(t, r), g(n.b));
		}),
		ir(() => {
			let e = Jr(() => n.m.map(h));
			return () => {
				for (let t of e) typeof t == `function` && t();
			};
		}),
		n.a.length &&
			ir(() => {
				(ao(t, r), g(n.a));
			}));
}
function ao(e, t) {
	if (e.l.s) for (let t of e.l.s) Q(t);
	t();
}
var oo = {
	get(e, t) {
		if (!e.exclude.has(t)) return e.props[t];
	},
	set(e, t) {
		return !1;
	},
	getOwnPropertyDescriptor(e, t) {
		if (!e.exclude.has(t) && t in e.props)
			return { enumerable: !0, configurable: !0, value: e.props[t] };
	},
	has(e, t) {
		return !e.exclude.has(t) && t in e.props;
	},
	ownKeys(e) {
		return Reflect.ownKeys(e.props).filter((t) => !e.exclude.has(t));
	}
};
function so(e, t, n) {
	return new Proxy({ props: e, exclude: t }, oo);
}
var co = {
	get(e, t) {
		let n = e.props.length;
		for (; n--; ) {
			let r = e.props[n];
			if ((p(r) && (r = r()), typeof r == `object` && r && t in r)) return r[t];
		}
	},
	set(e, t, n) {
		let r = e.props.length;
		for (; r--; ) {
			let i = e.props[r];
			p(i) && (i = i());
			let a = s(i, t);
			if (a && a.set) return (a.set(n), !0);
		}
		return !1;
	},
	getOwnPropertyDescriptor(e, t) {
		let n = e.props.length;
		for (; n--; ) {
			let r = e.props[n];
			if ((p(r) && (r = r()), typeof r == `object` && r && t in r)) {
				let e = s(r, t);
				return (e && !e.configurable && (e.configurable = !0), e);
			}
		}
	},
	has(e, t) {
		if (t === pe || t === he) return !1;
		for (let n of e.props) if ((p(n) && (n = n()), n != null && t in n)) return !0;
		return !1;
	},
	ownKeys(e) {
		let t = [];
		for (let n of e.props)
			if ((p(n) && (n = n()), n)) {
				for (let e in n) t.includes(e) || t.push(e);
				for (let e of Object.getOwnPropertySymbols(n)) t.includes(e) || t.push(e);
			}
		return t;
	}
};
function lo(...e) {
	return new Proxy({ props: e }, co);
}
function uo(e, t, n, r) {
	var i = !st || !!(n & 2),
		a = !!(n & 8),
		o = !!(n & 16),
		c = r,
		l = !0,
		u = void 0,
		d = () =>
			o && i ? ((u ??= Jt(r)), Q(u)) : (l && ((l = !1), (c = o ? Jr(r) : r)), c);
	let f;
	if (a) {
		var p = pe in e || he in e;
		f = s(e, t)?.set ?? (p && t in e ? (n) => (e[t] = n) : void 0);
	}
	var m,
		h = !1;
	(a ? ([m, h] = Rt(() => e[t])) : (m = e[t]),
		m === void 0 && r !== void 0 && ((m = d()), f && (i && nt(t), f(m))));
	var g = i
		? () => {
				var n = e[t];
				return n === void 0 ? d() : ((l = !0), n);
			}
		: () => {
				var n = e[t];
				return (n !== void 0 && (c = void 0), n === void 0 ? c : n);
			};
	if (i && !(n & 4)) return g;
	if (f) {
		var _ = e.$$legacy;
		return function (e, t) {
			return arguments.length > 0 ? ((!i || !t || _ || h) && f(t ? g() : e), e) : g();
		};
	}
	var v = !1,
		y = (n & 1 ? Jt : Qt)(() => ((v = !1), g()));
	a && Q(y);
	var b = K;
	return function (e, t) {
		if (arguments.length > 0) {
			let n = t ? Q(y) : i && a ? In(e) : e;
			return (F(y, n), (v = !0), c !== void 0 && (c = n), e);
		}
		return (Or && v) || b.f & 16384 ? y.v : Q(y);
	};
}
function fo(e) {
	return class extends po {
		constructor(t) {
			super({ component: e, ...t });
		}
	};
}
var po = class {
	#e;
	#t;
	constructor(e) {
		var t = new Map(),
			n = (e, n) => {
				var r = An(n, !1, !1);
				return (t.set(e, r), r);
			};
		let r = new Proxy(
			{ ...(e.props || {}), $$events: {} },
			{
				get(e, r) {
					return Q(t.get(r) ?? n(r, Reflect.get(e, r)));
				},
				has(e, r) {
					return r === he || (Q(t.get(r) ?? n(r, Reflect.get(e, r))), Reflect.has(e, r));
				},
				set(e, r, i) {
					return (F(t.get(r) ?? n(r, i), i), Reflect.set(e, r, i));
				}
			}
		);
		((this.#t = (e.hydrate ? Ri : Li)(e.component, {
			target: e.target,
			anchor: e.anchor,
			props: r,
			context: e.context,
			intro: e.intro ?? !1,
			recover: e.recover,
			transformError: e.transformError
		})),
			(!e?.props?.$$host || e.sync === !1) && hn(),
			(this.#e = r.$$events));
		for (let e of Object.keys(this.#t))
			e !== `$set` &&
				e !== `$destroy` &&
				e !== `$on` &&
				o(this, e, {
					get() {
						return this.#t[e];
					},
					set(t) {
						this.#t[e] = t;
					},
					enumerable: !0
				});
		((this.#t.$set = (e) => {
			Object.assign(r, e);
		}),
			(this.#t.$destroy = () => {
				Hi(this.#t);
			}));
	}
	$set(e) {
		this.#t.$set(e);
	}
	$on(e, t) {
		this.#e[e] = this.#e[e] || [];
		let n = (...e) => t.call(this, ...e);
		return (
			this.#e[e].push(n),
			() => {
				this.#e[e] = this.#e[e].filter((e) => e !== n);
			}
		);
	}
	$destroy() {
		this.#t.$destroy();
	}
};
function mo(e, t) {
	if ((He(`hydratable`), T)) {
		let t = window.__svelte?.h;
		if (t?.has(e)) return t.get(e);
		je(e);
	}
	return t();
}
var ho = t({
	afterUpdate: () => So,
	beforeUpdate: () => xo,
	createContext: () => yt,
	createEventDispatcher: () => bo,
	createRawSnippet: () => aa,
	flushSync: () => hn,
	fork: () => Cn,
	getAbortSignal: () => go,
	getAllContexts: () => Ct,
	getContext: () => bt,
	hasContext: () => St,
	hydratable: () => mo,
	hydrate: () => Ri,
	mount: () => Li,
	onDestroy: () => vo,
	onMount: () => _o,
	setContext: () => xt,
	settled: () => Gr,
	tick: () => Wr,
	unmount: () => Hi,
	untrack: () => Jr
});
function go() {
	return (U === null && $e(), (U.ac ??= new AbortController()).signal);
}
function _o(e) {
	(A === null && Ue(`onMount`),
		st && A.l !== null
			? Co(A).m.push(e)
			: ir(() => {
					let t = Jr(e);
					if (typeof t == `function`) return t;
				}));
}
function vo(e) {
	(A === null && Ue(`onDestroy`), _o(() => () => Jr(e)));
}
function yo(e, t, { bubbles: n = !1, cancelable: r = !1 } = {}) {
	return new CustomEvent(e, { detail: t, bubbles: n, cancelable: r });
}
function bo() {
	let e = A;
	return (
		e === null && Ue(`createEventDispatcher`),
		(t, r, i) => {
			let a = e.s.$$events?.[t];
			if (a) {
				let o = n(a) ? a.slice() : [a],
					s = yo(t, r, i);
				for (let t of o) t.call(e.x, s);
				return !s.defaultPrevented;
			}
			return !0;
		}
	);
}
function xo(e) {
	(A === null && Ue(`beforeUpdate`), A.l === null && tt(`beforeUpdate`), Co(A).b.push(e));
}
function So(e) {
	(A === null && Ue(`afterUpdate`), A.l === null && tt(`afterUpdate`), Co(A).a.push(e));
}
function Co(e) {
	var t = e.l;
	return (t.u ??= { a: [], b: [], m: [] });
}
export {
	fr as $,
	Li as A,
	m as At,
	gi as B,
	oa as C,
	mt as Ct,
	qi as D,
	Ie as Dt,
	Zi as E,
	ct as Et,
	Di as F,
	Zr as G,
	mi as H,
	Ci as I,
	Wr as J,
	Q as K,
	Ti as L,
	Hi as M,
	t as Mt,
	Ai as N,
	Ki as O,
	Fe as Ot,
	Oi as P,
	sr as Q,
	ki as R,
	ha as S,
	xt as St,
	ra as T,
	ut as Tt,
	pi as U,
	hi as V,
	di as W,
	Pr as X,
	Jr as Y,
	pr as Z,
	Sa as _,
	Ct as _t,
	uo as a,
	Kn as at,
	va as b,
	Tt as bt,
	io as c,
	In as ct,
	za as d,
	On as dt,
	ir as et,
	Ya as f,
	kn as ft,
	Oa as g,
	yt as gt,
	Aa as h,
	It as ht,
	fo as i,
	Gn as it,
	Ii as j,
	y as jt,
	Wi as k,
	v as kt,
	ro as l,
	Pn as lt,
	qa as m,
	Zt as mt,
	vo as n,
	Bn as nt,
	so as o,
	qn as ot,
	Ka as p,
	Nn as pt,
	Gr as q,
	_o as r,
	zn as rt,
	lo as s,
	Jn as st,
	ho as t,
	or as tt,
	$a as u,
	F as ut,
	xa as v,
	bt as vt,
	ia as w,
	ft as wt,
	_a as x,
	wt as xt,
	ya as y,
	St as yt,
	Ei as z
};
