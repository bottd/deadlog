...
test('an ability rail links to its own history and keeps the entity scope', async ({
	page
}) => {
	...
	await page
		...
		.getByRole('link', { name: /^Call Bell/ })
		.click();
	await expect(page).toHaveURL(/\/ability\/call-bell$/);
	await expect(
		page
			.getByRole('region', { name: 'Abilities' })
			.getByRole('link', { name: /^Call Bell/ })
...
test('toasts resolve semantic colors without Tailwind theme aliases', async ({
	...
	const toast = page.locator('[data-toast]').filter({ hasText: 'Copied to clipboard' });
...
test('hero abilities preserve slot order and scoped change counts', async ({ page }) => {
	...
	const buttons = rail.getByRole('link');
	...
	await rail.getByRole('link', { name: /^Call Bell(?:\s|$)/ }).click();

	await expect(page).toHaveURL(/\/ability\/call-bell$/);
	const scoped = page
		.getByRole('region', { name: 'Change History' })
		.locator('li:has(a[href="/change/2026/minor-07-28#call-bell-charge-time"])');
	await expect(scoped.getByText('1 change', { exact: true })).toBeVisible();
	await expect(scoped.locator('ul > li')).toHaveText([
		'Call Bell Charge Time increased from 6s to 7s'
	]);
	await expect(
		scoped.getByRole('heading', { name: 'Call Bell Charge Time' })
	).toBeVisible();
	await expect(scoped.getByRole('heading', { name: 'Doorway' })).toHaveCount(0);
});

test('hero ability cards share a height and top-align their icons', async ({ page }) => {
	await gotoApp(page, '/hero/wraith');
	const cards = page.getByRole('region', { name: 'Abilities' }).getByRole('link');
...
test('changelog contents surface ability icons on a card', async ({ page }, testInfo) => {
	...
	if (mobile) await page.locator('summary', { hasText: 'Contents' }).click();

	const toc = page.getByRole('navigation', {
		name: mobile ? 'Patch contents' : 'Table of contents'
	});
	...
	if (testInfo.project.name === 'desktop-chromium') {
		...
		const scrollState = await scrollContainer.evaluate((element) => {
			...
			return {
				// The sticky box holds the back link above the scrolling contents.
		...
		const back = page.locator('aside [data-patch-back]');
		await page.mouse.wheel(0, 4000);
		await expect(back).toBeInViewport();
		const [link, contents] = await Promise.all([back.boundingBox(), toc.boundingBox()]);
		expect(link!.y + link!.height).toBeLessThanOrEqual(contents!.y);
	}

	await gotoApp(page, '/change/2024/05-10');
	if (mobile) await page.locator('summary', { hasText: 'Contents' }).click();
	const historicalToc = page.getByRole('navigation', {
		name: mobile ? 'Patch contents' : 'Table of contents'
	});
	await expect(
...
test('hero entries list the changed items their players bought, linked to real sections', async ({
	...
	const block = page.locator('[data-share-block="related"]').first();
	await expect(
...
test('a hero entry with ability changes shows which ability players maxed first', async ({
	...
	const related = (await entry.locator('[data-share-block="related"]').boundingBox())!;
