var htmx = (function () {
	...
	function Gt() {
		...
		const n = te().querySelector('[hx-history="false" i],[data-hx-history="false" i]');
		if (!n) {
	...
	function en(e) {
		...
		if (t) {
			...
			if (ae(te().body, 'htmx:historyCacheHit', r)) {
				$e(r.historyElt, t.content, n, { contextElement: r.historyElt, title: t.title });
				_t(r.path);
	...
	function Ln(t, n, r) {
		...
		if (r) {
			if (r instanceof Element || typeof r === 'string') {
				return he(t, n, null, null, { targetOverride: w(r) || ve, returnPromise: true });
			} else {
				let e = w(r.target);
	...
	function jn(t, e) {
		...
		const l = H(n, /HX-Refresh:/i) && n.getResponseHeader('HX-Refresh') === 'true';
		...
		if (a) {
			fe(
				...
				le(
					{
						error:
							'Response Status Error Code ' + n.status + ' from ' + e.pathInfo.requestPath
					},
					e
				)
			);
